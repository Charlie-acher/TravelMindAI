...
> 文档版本：v0.4（三模型选择与按需研究协作）
> 更新日期：2026-09-21
> 状态：本文为目标设计；当前已实现工作流见08，当前代码与验收边界见08第12.20节
...
flowchart LR
    ...
    GRAPH --> REQ["主规划智能体：规划与委派"]
    REQ -->|"委派研究"| RES["Research 子智能体"]
...
上图按2026-09-21体验优先范围修订：主规划助手可委派独立研究助手，审核使用单次模型检查和程序规则。DeepSeek/Kimi/Qwen输入框选择覆盖文本与工具，首选优先、故障切备用；实现与真实验收见08第12.20节。私人附件仍不连到共享知识库入库链。
...
依赖由 `pyproject.toml + uv.lock` 管理，数据表由 Alembic 迁移管理。学习示例已移至项目外`E:/TravelMindAI-learning`，不由正式应用导入。规划目录按实际功能逐步建立。
...
- M4-C：增加单次模型审查、有限返工、持久checkpointer和interrupt恢复，保留已完成验收。
- M4-D：主规划助手自主委派Research，独立消息和工具循环，返回可核实地点与来源；审核保持独立单次检查。
...
| `review` | 运行代码规则和独立单次模型审核 | 通过/返工/用户决策 |
| `persist_draft` | 保存版本并生成响应摘要 | 等待用户确认 |

2026-09-21当前范围：仅接DeepSeek、Kimi、Qwen；输入框选择覆盖文本、JSON和原生工具。采用主规划智能体＋按需研究子智能体，审核保留程序规则和单次独立模型检查。不再将四个独立Agent作为本轮验收要求。当前实现与验证见08第12.20节、07同日记录。

进入研究/规划任务后，模型可以根据已获信息选择必要工具。身份、知识库范围、预算计算、版本写入和管理员操作仍由程序约束，不能由模型改写。工具上限与规划返工上限分别计数。
...
三个提供方可以复用 OpenAI-compatible 公共实现，但必须各自保留参数和异常映射；“接口看起来兼容”不等于工具调用、流式事件和结构化输出已经通过真实验证。

### 7.3 路由算法

输入框选择随请求冻结并贯穿主/子智能体的文本、JSON和工具调用。优先用户所选；故障时按DeepSeek → Kimi → Qwen固定顺序过滤已尝试、未配置、能力不兼容和熔断候选。同一执行切换后保持备用，下一轮重新优先用户选择。不用权重/延迟评分随机替换健康首选；现有M5-B加权实现将在M5-C替换。正文已展示后不得跨模型续写。手动停止换模、持久等待恢复与幂等规则见修订设计第4节。

### 7.4 错误与降级矩阵

| 错误 | 同模型重试 | 跨提供方降级 | 熔断计数 |
|---|---:|---:|---:|
| 连接错误/超时 | 有备用则不先重试 | 是，立即尝试可用备用 | 是 |
| 429 | 同提供方遵守 `Retry-After`，不阻塞备用 | 是 | 是 |
| 500/502/503/504 | 有备用则不先重试 | 是 | 是 |
| 空响应/JSON 校验失败 | 修复 1 次 | 是 | 是 |
| 401/403 | 否 | 可跳过该提供方 | 立即打开配置告警 |
| 400 参数错误 | 否 | 否，修复参数/配置 | 否 |
| 内容安全拒绝 | 否 | 否 | 否 |
| 用户取消 | 否 | 否 | 否 |
...
model_choice_target:
  default: deepseek
  selectable: [deepseek, kimi, qwen]
  fallback_order: [deepseek, kimi, qwen]
  selection_scope: conversation
  frozen_for: execution
```

这是目标策略示意，不是当前可直接复制的配置字段；实际配置以backend/.env.example和实现后的08为准。真实型号放在后端配置中，前端不得提交任意端点或型号。视觉和Embedding保持独立配置。
...
飞书使用 FAISS/Chroma，本项目继续使用 Milvus，不同时维护多个向量库。当前文本模型范围为DeepSeek、Kimi、Qwen；MiniMax退出本轮范围，不新增其他提供方。Redis/Celery在出现实际需求时引入。历史取舍见[参考文档对照](./05-参考文档实施取舍.md)，与本次模型和Agent范围冲突处以本次修订为准。
