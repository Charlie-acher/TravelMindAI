...
def review_plan(message: str, requirements: TravelRequestExtraction,
    ...
    raw = model.generate_json([
        {"role": "system", "content": "你是独立审查员，不参与生成。只返回JSON。"
         ...
         "不要重新做时间和金额运算，也不因11点与11:00等同义写法要求返工。"
         "仅修改第二天的要求不作用于第一天；保持其他天原样本身不是错误。"
         "只指出证据明确的需求遗漏或冲突，偏好上的可选改进不能变成阻断条件。"
         "票价、开放时间及未查到的路线已提示限制，不因此要求反复返工；"
         "路线是否查到以每段route状态为准。"
         "少走路、舒适等偏好按整体安排理解，不要求逐项证明。"
         "明确要求尚缺证据但草稿已标明未确认，不等于已知违反，不因此拦住可讨论草稿；"
         "只有证据明确表明安排违反要求，或把未知说成已满足，才要求修改。"
