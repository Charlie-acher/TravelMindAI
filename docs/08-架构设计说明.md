...
> 当前源码核对日期：2026-09-22。对话工作区生产接入、公开过程持久化、标题来源、会话统计、个人文件及知识库检索测试见12.26；此前章节的阶段验收记录保留历史含义，相关旧限制由12.26更新。

> 2026-09-21当前范围：仅接DeepSeek、Kimi、Qwen；输入框选择覆盖文本、JSON和原生工具。采用主规划智能体＋按需研究子智能体，审核保留程序规则和单次独立模型检查。不再将四个独立Agent作为本轮验收要求。当前实现与验证见08第12.20节、07同日记录。

### 两个智能体实际位置（2026-09-21源码核对）

| 文件 | 当前职责 |
|---|---|
| `services/itinerary/graph.py::plan_trip` | 主规划循环，通过`delegate_research`按需委派；自己安排和修改行程 |
| `services/itinerary/research.py::run_research` | 独立研究消息与工具循环，查资料和地图，返回已核实地点及缺口 |
| `services/itinerary/review.py` | 程序规则＋独立单次JSON审核，不称为第三个自主Agent |
| `services/chat/workflow.py` | 持久检查点、有限返工、等待与发布，图节点不是Agent数量 |
| `services/chat/service.py` / `services/chat/dining.py` | 保留自然聊天业务分支及既有周边查询助手；不强制所有问题走研究 |
| `llm/gateway_client.py` / `tool_model.py` | 所选模型贯穿文本和原生工具；请求内成功备用保持一致 |

普通问答保持原业务流程；本次两智能体协作只扩展规划路径，不新增总控平台。
...
首次成功聊天先保存原话去多余空白后的前40字作为可用回退标题，再由模型总结短标题；手动名称优先，失败保留回退值，详见12.26。后续成功轮次更新`updated_at`。用户历史按`updated_at/id`倒序游标分页并支持服务端标题搜索，浏览器`sessionStorage`只记当前选择和待确认发送，服务端历史才是找回旧会话的依据。
...
**`sessions`：一段旅行会话。迁移：`0001_business_tables`，归属列由0008/0010增加，标题来源由0016增加。以下当前字段均不可空。**

| 字段 | 数据库类型 | 含义、默认值及约束 |
|---|---|---|
| `id` | UUID | 主键；应用生成 |
| `thread_id` | UUID | 会话工作流编号；应用生成，唯一；持久检查点见12.17 |
| `user_id` | UUID | 0008过渡可空、0010当前非空；无默认值，外键→`users.id`，由服务端登录身份写入 |
| `title` | VARCHAR(200) | 会话标题；无数据库默认值 |
| `title_source` | VARCHAR(20) | 数据库默认`manual`；CHECK `ck_sessions_title_source`限定pending/generating/auto/manual/fallback；旧会话按manual保留 |
...
```mermaid
flowchart TD
  A[完整需求与旧行程] --> C[create_agent：模型理解偏好并查询资料]
...
**规则与局限：**沿用2～5天、1～8人和单目的地。程序检查天数完整、同天不重复、全程地点不重复、明确排除项、活动在08:00～21:00、每项30～240分钟、前后不重叠及至少30分钟交通预留；轻松/均衡/紧凑每天活动加预留分别不超过360/480/600分钟。交通方式与预留不等于实际路线耗时，开放时间、预约和票价仍待核实。其他硬条件缺依据时允许生成标明“未确认”的讨论草稿，不宣称已经满足；2026-09-21已移除检索前的一刀切阻断，见12.21。只有用户明确点名“第N天”且旧计划的天数/目的地/开始日期适配时，程序只接收指定天并保留其余活动；未点名时可重新编排全程，日期按当前需求重算。

预算由现有Decimal服务使用`demo-cny-v1`计算，舒适住宿偏好选comfort，其余economy；超演示预算拒绝该计划并在剩余轮次内反馈调整，不将模型金额作为结果。预算与路程都不是实时报价或可达性验证。

`rules.start_constraint`识别明确某天开始时间，例如“第二天11点开始游玩”，程序要求当天第一项活动的start_time精确匹配；超出旅行天数或未匹配时拒绝计划，不能把该条件只写成一句承诺。到达、返程和模糊时段不当作开始时间；无障碍等其他未核实硬条件在草稿中逐项说明缺口，模型结合已有证据安排；已知违反条件仍需修正。
...
2026-09-20核对：地图/周边与行程内部继续使用 LangChain create_agent。M4-C 外层由 `services/chat/workflow.py` 的 StateGraph 串联候选生成、独立审查、最多两次返工、interrupt 和保存，PostgreSQL 检查点见12.17。内部工具 Agent 明确使用 checkpointer=False：它们的临时证据在闭包中，只由外层完整候选形成恢复边界，不能恢复半个工具闭包。

当前调用顺序：鉴权与历史幂等/版本检查 → 持久图 generate（读取历史、附件、统一理解、原普通问答或工具规划）→ review（仅候选草稿调用独立 Reviewer）→ publish（原业务事务）。缺项或取舍进入 wait，浏览器新回复用同一 run_id 恢复。普通自然聊天继续原回答业务，外层包装不意味着所有聊天都是工具 Agent。原附件归属、私人证据、局部修改与撤销不变。

`services/itinerary/graph.py` 管理模型与工具循环，`tools.py` 包装只读检索/地图，`rules.py` 核算确定规则，`review.py` 独立重算并另发模型审查。`requirement/history.py` 和 `trip_service.py` 继续负责版本、幂等、原子保存与会话删除。图状态不保存客户端或连接。0014 与官方 PostgresSaver 3.1.2 配套，已显式迁移；不是启动时自动 setup。恢复不等于外部查询恰好一次，提交正确性仍依赖业务事务。
...
此前浏览器真实登录专用账号恢复杭州和长沙草稿，核对路线按钮只填问题；重新选择同会话长沙PDF发送“只读取附件”时，连接中已显示附件在用户气泡上方，输入及移除附件按钮均清空；已识别缓存可复用。当时长附件明细默认折叠；当前已按12.13移除用户侧识别清单。此前文字小样本的指定日替换、幂等和撤销证据保留在07历史节。真实票价和逐段路线自动核对仍待后续；M4-C独立审查及持久恢复见12.17。
...
### 12.16 M4-B 图片端到端验收与匹配修补（2026-09-20）

本节补齐12.12/12.15尚未覆盖的清晰路线图片首次上传到新行程的浏览器实测。调用链仍为上传→MinerU完整解析→文字提取与Qwen图像补读→合并私人依据→用途判断→同一规划图和百度地点核对→规则校验→原子保存；HTTP/JSON、表结构与迁移版本0013均未变化。

- `attachment/analysis.merge_analyses`比较各份识别结果的完整地点序列与order；全部一致才保留顺序，有缺项、冲突或不同批次内容则撤回全局次序，不把独立路线强行连接。后续`finish_analysis`仍检查连续编号，识别不授予地图核实状态。既有识别缓存和历史消息不回写。
- `baidu.BaiduMaps.lookup`先核对名称、同城和主体类别，再优先精确原名；没有精确原名才使用原有官方别名和严格末尾景区称谓。多个精确同名结果仍返回ambiguous。真实花港观鱼与花港公园别名碰撞可区分，真实白堤景点与同名道路仍补问。
- 原有required覆盖与顺序规则、指定日保护、预算校验、权限及短事务不变。不会为了让某张图通过而删除未核实必经点或自动改用别的地点。

真实验收样本为专用账号自制四点杭州PNG：断桥残雪→平湖秋月→雷峰塔→花港观鱼。新会话发送前`uploaded/analysis=null`且历史为空，浏览器上传并首次发送，页面31秒完成两天草稿v1。实际调用MinerU、DeepSeek、百炼Qwen及百度MCP，无识别结果注入；四点按1～4顺序保存，每点带本会话私人来源及BD-09地图依据。上海出发、1人、4000元、经济住宿和每天9点开始均保留。

同一浏览器第二天改11点生成v2，用时8秒；HTTP核对第一天完整JSON和预算等于v1，消息幂等重试不新增revision。页面撤销为v3，完整计划及旅行条件逐字段等于v1；刷新并再次进入规划页恢复三轮历史和v3。再上传成都文字附件，只读用时6秒且不改需求；继续用其替换杭州第二天时3秒返回城市冲突，旧草稿不变。数据库仅有三个行程版本，撤销请求重放仍返回原v3且对话revision保持5。浏览器控制台未记录错误。

真实接口同时验证跨会话附件提交404、跨用户历史/原件404、损坏PNG422且不落附件记录；上传原件回读SHA-256一致，私人解析正文缓存存在，样本未进入共享documents。地图歧义样例不计为成功规划：原六点图在白堤处安全拒绝，另一早期请求暴露花港观鱼别名碰撞，修复后重新新建会话完成首次识别全链，未将缓存重试冒充首次成功。

最终全量801 passed、0 skipped、2条既有依赖警告，真实隔离PostgreSQL/Milvus；mypy 94文件、修改范围Ruff、前端三组回归、Vue类型检查和Vite构建通过。该证据只证明这些样例，复杂/模糊路线、广泛规划稳定率及16页PDF一次浏览器长请求成功率仍未充分验收。交通预留和演示预算不升级为真实路线耗时或实时价格；本轮后续M4-C实现与验收见12.17。
### 12.17 M4-C 独立审查与持久编排（2026-09-20）

两种会话消息入口调用 `run_saved_workflow`。稳定执行号由会话与原消息 UUID 推导，归属由认证入口和 `chat_workflows.session_id` 双重核对。同会话 PostgreSQL advisory lock 非阻塞保护图推进，最终写入仍由原行锁、消息幂等和双版本校验保护。

```mermaid
flowchart LR
  H[历史幂等与版本检查] --> G[generate：原聊天生成候选]
  G --> R[review：规则重算与独立模型审查]
  R -->|通过或普通聊天| P[publish：原事务保存]
  R -->|可修正，最多两次| G
  R -->|缺项、取舍或达到上限| W[wait：持久 interrupt]
  W -->|用户补充条件| G
  W -->|再想想：保留候选，不继续追问| W
  W -->|采用合规候选或取消| P
  C[(PostgreSQL 检查点)] --- G
...
`TravelGraphState` 保存 payload、candidate（回复、候选、旧草稿和版本）、attempts、review、feedback、route、result、cancelled、wait_reply；不序列化模型、连接或工具闭包。wait_reply为可缺省字符串，只用于暂缓后的自然回复，旧检查点缺省为空；无新增SQL字段。候选仅经规则和独立审查后进入原保存事务。等待消息另存为历史；恢复携带执行号及 continue/accept/cancel，旧轮次不可继续，失败重试必须复用原请求。正常输入以continue传入，由模型结合上一轮问题识别继续、结束、暂缓或采用；暂缓回到wait。accept仅允许程序校验通过且审查结论为choice的可见候选，详见12.21。

官方依赖为 `langgraph-checkpoint-postgres 3.1.2`，请求不调用 setup；0014 固定安装官方结构版本 0～9，并增加会话级联外键。正式迁移和真实验收记录在 07；下列是源码契约。

#### SQL 字段字典（0014_chat_workflows）

| 表 | 字段及类型 | 空值与默认 | 含义及约束 |
|---|---|---|---|
| chat_workflows | id text | 非空，无默认 | 稳定图执行 UUID 字符串，主键 |
| chat_workflows | session_id uuid | 非空，无默认 | 外键 sessions.id，ON DELETE CASCADE |
| chat_workflows | message_id uuid | 非空，无默认 | 原始消息编号，与 session_id 联合唯一 uq_workflow_message |
| chat_workflows | payload_json jsonb | 非空，无默认 | 原始请求，重试核对依据 |
| chat_workflows | created_at timestamptz | 非空，now() | 执行创建时间 |
| checkpoint_migrations | v integer | 非空，无默认 | 官方结构版本主键，初始 0～9 |
| checkpoints | thread_id text | 非空，无默认 | 外键 chat_workflows.id，级联删除，单列索引 ix_checkpoints_thread_id |
| checkpoints | checkpoint_ns text | 非空，空串 | 图命名空间 |
| checkpoints | checkpoint_id text | 非空，无默认 | 检查点编号；与 thread_id/checkpoint_ns 联合主键 |
| checkpoints | parent_checkpoint_id text | 可空，无默认 | 上一个检查点编号，官方逻辑关联，无 SQL 外键 |
| checkpoints | type text | 可空，无默认 | 官方兼容类型字段 |
| checkpoints | checkpoint jsonb | 非空，无默认 | 渠道版本及图状态结构 |
| checkpoints | metadata jsonb | 非空，{} | 节点步骤等执行元数据 |
| checkpoint_blobs | thread_id text | 非空，无默认 | 外键 chat_workflows.id，级联删除，索引 ix_checkpoint_blobs_thread_id |
| checkpoint_blobs | checkpoint_ns text | 非空，空串 | 图命名空间 |
| checkpoint_blobs | channel text / version text | 非空，无默认 | 状态渠道与版本；与 thread_id/checkpoint_ns 组成四字段主键 |
| checkpoint_blobs | type text | 非空，无默认 | 官方序列化类型 |
| checkpoint_blobs | blob bytea | 可空，无默认 | 序列化渠道内容，空表示该版本无值 |
| checkpoint_writes | thread_id text | 非空，无默认 | 外键 chat_workflows.id，级联删除，索引 ix_checkpoint_writes_thread_id |
| checkpoint_writes | checkpoint_ns text | 非空，空串 | 图命名空间 |
| checkpoint_writes | checkpoint_id text / task_id text / idx integer | 非空，无默认 | 所属检查点、任务与写入序号；与 thread_id/checkpoint_ns 组成五字段主键 |
| checkpoint_writes | channel text | 非空，无默认 | 写入渠道，包括中断或错误记录 |
| checkpoint_writes | type text | 可空，无默认 | 写入值的序列化类型 |
| checkpoint_writes | blob bytea | 非空，无默认 | 待合并节点写入 |
| checkpoint_writes | task_path text | 非空，空串 | 官方任务路径 |

新表无额外 CHECK 约束。检查点之间使用官方逻辑关联，只有执行归属使用外键；各复合主键自带唯一索引。Alembic 元数据包括全部官方表，避免自动迁移误删。

#### HTTP 与历史 JSON 扩展

`SavedRequirementMessage.workflow_resume` 可空，非空时为 `{run_id: UUID, action: continue|accept|cancel}`。响应 `workflow_request` 回显该恢复选择，保证重试不能更换动作。`workflow` 可空，包含 run_id、status(waiting/completed/cancelled)、attempts(0～3)、issues(数组，默认空)、can_accept(默认false)、preview(可空 TravelPlan)。preview 仅用于可采用候选，尚无已提交行程版本。缺省新字段兼容旧历史，JSON 引用不是独立 SQL 外键。

节点完成后同步写检查点；进程重启后由同一请求重试或用户继续触发恢复。若退出发生在生成节点内部，该节点的只读模型/工具查询会重做；不声称恢复到单次工具调用内部。publish 可能重放，业务消息编号幂等防止重复行程。没有新增后台自主继续任务，也不加入跨会话长期偏好。

恢复失败保护：浏览器保留未确认的接续原请求，运行锁409不丢编号，可点击“重试上次接续”；编辑或附件变更不得覆盖它。若客户端已丢失原编号，仍可在当前等待轮、会话锁和版本检查通过后明确取消，图跳至取消提交而不采用候选。取消恢复为本次执行前的需求快照。

**本节运行核对：**0014已正式执行并通过Alembic check；818项全量回归通过，真实浏览器暂停/重启/接续、修改/撤销/取消与刷新完成。详细样本、失败后修补及边界见07的M4-C最终验收结论。内部Agent关闭检查点并显式覆盖async运行模式，避免当前LangGraph继承外层sync造成异常；外层仍以sync提交检查点。

### 12.18 M5-A：四提供方文本适配器（2026-09-20）

> 历史阶段记录：当前模型范围、入口、路由和统计以12.20为准；本节验收数字仅属于当时版本。

本节描述已进入源码的适配基础，尚不是完整ModelGateway。业务入口保留DeepSeekClient，新提供方不自动参与聊天、附件提取或工具调用。没有新增数据库表、HTTP接口、会话字段或前端状态。

```mermaid
flowchart LR
    Env[backend/.env独立文本配置] --> Settings[Settings / ProviderSettings]
    Settings --> Build[build_chat_model]
    Build --> Old[DeepSeekClient原业务封装]
    Old --> Business[M4需求/知识/行程/审查]
    Settings --> Pool[configured_adapters]
    Pool --> Adapter[ProviderAdapter]
    Adapter --> Build
    Adapter --> Result[ModelResult / ModelStreamEvent / ProviderError]
```

**配置契约：**DeepSeek继续读取`TRAVELMIND_DEEPSEEK_API_KEY/MODEL/TIMEOUT_SECONDS`，端点新增`TRAVELMIND_DEEPSEEK_BASE_URL`，默认原DeepSeek地址。另三家通过`TRAVELMIND_MODEL_PROVIDERS__QWEN/KIMI/MINIMAX__API_KEY/BASE_URL/MODEL/TIMEOUT_SECONDS`逐项填写，前缀中的提供方名和字段名分别替换，不是完整字面变量名。密钥为空则排除；有密钥而缺地址或型号，构造适配器时报configuration。模型和地区端点无猜测默认值；不借用VISION或EMBEDDING设置。超时必须有限且大于0。

**调用契约：**`ModelRequest(messages,json_object=False,max_output_tokens=2048)`仅承载文本。`complete`返回提供方、响应实际型号（缺失时使用配置型号）、正文、可空input/output/total tokens和elapsed_seconds。`stream`返回正文增量，完整结束并校验后返回completed和完整结果；缺失finish_reason、截断、空正文、非法JSON或非对象均拒绝。推理字段不发给页面，同块content_filter先拒绝再输出；此前已发送的正文无法撤回。token没有报告时为null，不写成0。

**错误契约：**configuration、timeout、network、rate_limit、unavailable、authentication、quota_exceeded、invalid_request、refused、invalid_output；错误只包含安全类别与提供方。流失败的started=true表示已发正文，后续网关必须禁止跨模型拼接。401/403归authentication，402归quota_exceeded，429归rate_limit，5xx归unavailable；本层与SDK均不自动重试。异常不外显远程响应体、密钥或提示词。

新适配器使用CheckedChatOpenAI在原始响应转换处保留错误语义：MiniMax input/output_sensitive或1026/1027归refused、1001归timeout、1002/1039/1041/2045/2056归rate_limit、1004/2049归authentication、1008归quota_exceeded、1000/1024/1033归unavailable，其余非零业务码归invalid_request；普通SSE的delta.refusal也在SDK丢字段前拒绝。依据[官方错误码](https://platform.minimax.io/docs/api-reference/errorcode)。本实现覆写安装版LangChain两个转换方法，依赖升级需重跑真实SDK契约测试；旧DeepSeekClient默认仍使用原ChatOpenAI及原业务异常。

**提供方差异：**DeepSeek和可关闭思考的Kimi K2文本型号发送thinking.disabled；Qwen型号需支持enable_thinking=false；MiniMax发送reasoning_split=true，防止把推理内容当正文。前三家JSON请求带response_format=json_object；MiniMax的公开兼容契约未明确支持该字段，本步只加JSON提示并本地解析对象，不承诺原生schema约束。新适配器不把另三家直接交给create_agent，工具调用与思考历史兼容性留到业务能力路由接入时验证。

官方依据：[DeepSeek](https://api-docs.deepseek.com/api/create-chat-completion/)、[Qwen兼容接口](https://help.aliyun.com/zh/model-studio/compatibility-of-openai-with-dashscope)、[Qwen思考参数](https://help.aliyun.com/zh/model-studio/deep-thinking)、[Kimi](https://platform.kimi.com/docs/api/chat)、[MiniMax](https://platform.minimax.io/docs/api-reference/text-openai-api)，核对日期2026-09-20。

真实验收：正式.env只有DeepSeek文本配置，17:53及修复后17:59通过deepseek-v4-pro的普通文本、JSON对象和SSE短请求；新适配器和既有客户端共用模型构造。另三家只有模拟HTTP契约证据。负载选择、并发限制、熔断、fallback、业务schema修复尚未接入，不能将本节描述为四模型网关完成。

自动验证：107项适配契约包含真实SDK解析与四家模拟HTTP；修复后全量925项通过、0跳过，mypy 100文件和修改范围Ruff通过。现有服务未重启，当前应用仍使用M4已验收运行实例；本步真实请求由独立验证进程加载新适配器执行。

### 12.19 M5-B：文本能力路由与共享熔断（2026-09-21）

> 历史阶段记录：当前模型范围、入口、路由和统计以12.20为准；本节验收数字仅属于当时版本。

本节取代12.18中“新适配器尚未接业务”的当前状态。普通需求、会话HTTP/SSE、附件长文本抽取改用GatewayClient，完整数据库持久化仍由M4负责；无新表、迁移或会话字段。原生行程和周边Agent继续通过`.model`使用已验收DeepSeek，不受下述文本网关并发/熔断计数控制，M5-C再处理。

```mermaid
flowchart TD
    API[普通HTTP / 会话HTTP / SSE工作线程] --> Client[GatewayClient 请求级HTTP连接]
    Tasks[需求与附件extract / 资料research / 审查review / 自然回复chat] --> Client
    Client --> GW[ModelGateway 显式能力候选]
    GW --> State[app.state.model_gateway 进程共享状态与锁]
    State --> Pick[权重 × 平滑成功率 ÷ 1加平均耗时]
    Pick --> Adapter[ProviderAdapter 一次请求]
    Adapter --> Valid[原业务字段与引用校验]
    Valid --> Save[M4工作流与历史提交]
    Adapter --> Fault[错误分类]
    Fault --> Retry[最多一次同模型重试]
    Retry --> Fallback[未输出正文才可切换候选]
    Client --> Native[model属性 原生DeepSeek工具调用]
```

**配置及授权：**`Settings.model_gateway`读取`TRAVELMIND_MODEL_GATEWAY__...`。routes是能力到提供方/正权重的映射，默认各能力仅deepseek；未写入能力路由或未配文本密钥者不参与。一次候选被执行后加入本次排除集合，不再来回切换；四提供方最多8次网络尝试。plan键预留但当前工具调用不经此路由，管理员返回`native_tool_calls_included=false`。Qwen真实文本已验证，但本机正式routes仍只有DeepSeek。

**能力与业务边界：**JSON默认extract；资料回答用`capability_scope("research")`，独立审查用review；上下文退出恢复，互不串会话。自然文本固定chat。JSON语法层对象解析失败仅加一次格式提醒，旅行schema、引用与内容修复仍由原业务最多一次处理。两个层次上限分别计算，不能把8次适配器上限说成整轮业务最多8次。`prepare_json_messages`共用旧历史转换，保留角色、顺序及内容；不截断必留输入。

**选择与并发：**GatewayState在create_app时创建，所有请求/工作线程共享；HTTP连接和客户端仍按请求关闭。锁只保护状态计算和名额，不包住远程调用。分数=配置权重×`(窗口成功数+1)/(窗口样本数+1)`÷`(1+窗口平均执行秒数)`，随后加权抽签，单次选定后重试保持同一提供方。默认每提供方最多4个执行名额；满载选择其他已授权候选，无候选立即503，不无限排队。单个worker一份状态，多worker不共享，重启清空。

**重试与熔断：**timeout/network/429/5xx/invalid_output允许一次重试；默认0.25～0.5秒抖动退避。Retry-After支持数字与HTTP日期，不超过默认5秒时按其等待，超过则跳过同模型重试。401/403、余额不足或执行期配置错误不重复撞原提供方，该进程禁用其候选；构造阶段已授权但缺地址/型号的配置直接报错，需纠正配置。400与内容拒绝直接终止，不跨提供方规避。SDK重试关闭。

一次提供方执行（包括一次可能的重试）最终可用性失败计一次熔断，默认连续3次打开、冷却30秒。打开前已经在途的旧请求只计指标，不能提前关熔断；必须等旧名额全部释放，才允许唯一Half-open探测。探测成功Closed，失败或探测中取消重新冷却；鉴权失败Disabled。并发交错已先复现后修复并独立复查。

**流式与异常：**网关自身跟踪首段正文，不能只依赖适配器started。发送正文后发生任何故障，立即停止，不重试或切换模型拼接；迭代器关闭及异常路径都释放适配器流和并发名额。SSE保留reset/draft/done/error协议，失败不会发送done。新增fallback事件仅包含from_alias/to_alias/reason_category，当前前端忽略该可选事件，不新增页面控件。

**可观测性：**`GET /api/v1/requirements/gateway-status`仅管理员可用，返回进程内routes、安全健康快照、scope与request_id。每提供方含state、probe_ready、in_flight、calls、retries、success_rate、p95_seconds、window_count；顶层fallback_count累计真实切换。最近窗口默认10次提供方执行，耗时含同提供方重试/等待，成功率按最终结果计算；缺少样本时null。日志仅写别名与错误类别，不含密钥、端点、提示词或正文。不是跨重启的监控数据库，也不包含原生工具调用。

**验收与部署：**954项全量后端通过、0跳过，mypy102文件、修改Ruff通过。正式.env的deepseek-flash与qwen3.8-max普通/JSON/SSE短提示通过；单独验证进程注入主错误→真实Qwen成功，不等于供应商真实故障或目标降级率达标。当前后端已更新，真实Cookie账号完成SSE缺项等待、历史恢复与同编号幂等，专用数据清理。详细时间与边界见07 M5-B；Kimi/MiniMax真实接通、工具网关、统一schema修复、开发演示开关与量化故障评测仍未完成。


### 12.20 三模型选择、按需研究与路线核实（2026-09-21）

本节替代12.18/12.19关于当前提供方、入口和路由的描述。文本范围仅DeepSeek、Kimi、Qwen；视觉与Embedding仍使用独立专用配置。无新增SQL表、索引或迁移，数据库仍为0014。自然聊天不强制补全行程表单，也不强制委派。

```mermaid
flowchart TD
    UI[输入框选择模型] --> API[消息请求 冻结selected_provider]
    API --> WF[持久工作流 generate review wait publish]
    WF --> Main[graph.py 主规划智能体]
    Main -->|按需一次delegate_research| Child[research.py 独立研究循环]
    Child -->|已核实地点 来源及缺口| Main
    Main --> Rules[日期 节奏 修改范围 预算及路线校验]
    Rules --> Review[程序复核与单次模型审核]
    Review --> Save[原子保存草稿与消息]
    Main --> Native[GatewayChatModel 原生工具]
    Child --> Native
    Native --> GW[共享模型网关]
    Review --> GW
    API --> Text[自然回复与结构提取]
    Text --> GW
    GW --> Providers[首选优先 DeepSeek Kimi Qwen]
    Save --> Card[实际模型标签及逐段路线状态]
```

**模型选择：**消息`selected_provider`仅允许三个别名，缺省deepseek，发送时冻结。完整配置的三家都可选；status的`configured`只表示配置齐全，不承诺实时可用。请求首选健康时固定使用；其后按DeepSeek、Kimi、Qwen去重选择可用备用。本轮某备用成功后，后续提取、规划、研究及审核保持该备用，下一条仍从用户首选开始。显式选择覆盖旧能力权重；旧内部调用未传选择时继续兼容routes。MiniMax适配和配置已移除。

**故障与工具：**超时、网络、限流和服务故障不在原提供方重试，直接尝试下一家；格式错误保留一次修复。400和内容拒绝不跨模型规避。沿用进程共享并发、熔断和错误脱敏；原生工具完整接收参数才交LangGraph执行，保留ToolMessage编号，未完整执行的调用可换模型。正文已开始后不切换拼接。fallback事件显示切换，回复`used_providers`显示本次实际调用过的提供方。原生响应记录可空token计数和耗时；尚无跨请求费用账单或质量排名。

**停止与恢复：**`POST /sessions/{sid}/requirement-messages/{mid}/stop`标记本进程SSE工作线程；`GET .../{mid}/execution`确认线程退出。均继承登录、会话归属与写请求同源保护。停止在模型/工具边界生效，不强行中断在途外部HTTP；前端等待确认后重新读历史，保留原输入和附件，允许换模重发。已提交结果由历史恢复，未确认停止不宣称成功。普通失败原样重试保留编号；改变模型或内容使用新编号，版本保护继续生效。检查点接续请求必须原样重试或取消，不能擅自替换进行中的接续。取消状态和健康信息目前仅支持单进程；重启清除，不冒充多worker共享控制。

**研究边界：**主规划最多6轮，研究子任务最多5轮且每次规划至多委派一次；研究与父任务共用12次检索/地点查询预算。子任务有独立system prompt、消息和工具循环，无发布/删除权限；只回传工具实际核实的地点和缺口。父任务根据结果继续排程。`agent_tasks`记录本次候选研究摘要及父任务编号；父编号标识规划执行，非数据库外键。失败重试的过程仅在运行日志中可见，不承诺完整永久Agent审计平台。已有周边查询助手继续用于地图问题，不将其重新包装成必经角色。

**路线边界：**`BaiduMaps.route`通过现有只读MCP `map_directions`查询walk/transit/taxi（分别walking/transit/driving），输入必须BD-09，参数顺序纬度、经度；秒向上取整为分钟。只查本轮变更日相邻活动，每次规划最多10条不同路线并复用结果；坐标缺失、坐标系不符、无路线或工具失败保留未知。估时超过预留时间拒绝候选，让主规划调整时间、交通或地点。查得路线只代表查询时估计，不保证未来到达时间；旧未变更日保持原样。预算仍为演示价格，不宣称实时票价、酒店价格或开放时间。

**既有JSON字段扩展字典（无SQL结构变化）：**

| 所在快照 | 新字段/类型 | 空值、缺省与约束 |
|---|---|---|
| `chat_workflows.payload_json` | `selected_provider`: string | 不可null；缺省deepseek；仅deepseek/kimi/qwen；同消息编号改变选择返回冲突 |
| 消息`response_json` / 图候选 | `selected_provider`: string | 不可null，旧数据读取缺省deepseek |
| 消息`response_json` / 图候选 | `used_providers`: string[] | 不可null；缺省[]表示旧记录或未调用；按实际成功顺序去重 |
| 消息`response_json` / 图候选 | `agent_tasks`: object[] | 不可null；缺省[]；程序写入研究执行摘要 |
| `agent_tasks[]` | `task_id,parent_task_id,agent,status`: string；`queries`: int；`elapsed_seconds`: number | 程序生成UUID字符串；agent为research，status为completed/incomplete/failed；无SQL外键 |
| 行程快照`days[].activities[]` | `route`: object/null | 缺省null，旧数据兼容；活动首项无入站路线 |
| `route` | `status`: estimated/unavailable；`provider`: baidu | 非null；provider缺省baidu |
| `route` | `duration_minutes`: int/null；`distance_m`: int/null | 未知null；分钟>=1，米>=0；模型不能自行提交估时 |
| `route` | `checked_at`: ISO datetime | 非null，程序查询时UTC时间；不代表未来实时交通 |

既有主外键、唯一约束、索引和级联删除不变；本节新增JSON属性没有数据库server default，其缺省由Pydantic读取/程序构造处理。

**验证范围：**自动回归、三家基础真实调用、数据库与浏览器结果独立记录在07。当前服务加载本节源码；未把短提示smoke当作大规模规划质量、P95或故障降级率达标。下一步优先固定旅行质量集与真实多轮样本，只有测量证明收益才扩展更多独立Agent。


### 12.21 自然接续与减少规划阻断（2026-09-21）

截图问题来自两条链：规划缺项被转换成带操作按钮的持久等待；“少走路、节奏舒适”被旧抽取误写为硬条件，`plan_trip`又在检索前统一拒绝尚未核实的硬条件。这里的“缺依据”只是程序分类结果，并不代表已经查询后发现景点不适合。

当前先理解再安排：`requirement/prompt.py`区分舒适偏好与明确限制；`merge.py`仅兼容修正旧记录中的少数明确舒适短语，移入interests，未指定pace时设为relaxed。数字步行上限、无障碍等具体要求保留。`itinerary/graph.py`先让规划模型检索、选点并按偏好安排；`rules.py`把尚无证据的要求写成逐项未确认提醒，Reviewer不因未知而反复返工，也不能把未知说成已满足。地点来源、明确排除、时间冲突、预算核算和指定日保护仍保留。

前端普通输入继续携带当前等待执行号。`planning_reply_action`单独一次JSON模型调用，结合上一轮问题、要求及候选可用状态理解整句话：结束→cancel；再想想→defer并再次interrupt；补充/修改→continue重新生成；明确采用可用候选→accept。无候选时“先出一版”继续规划，不伪造候选。原始请求不改写，历史幂等比较和失败原样重试保持一致。wait_reply只存检查点JSON，无SQL迁移、表关系或索引变化。

页面取消补充条件/保留现状按钮及过期等待提示，保留可折叠候选内容。旧历史显示时移除固定引导尾句，不改写数据库历史。自然结束取消本轮未发布候选并恢复执行前需求，不删除既有历史或已发布草稿；用户不再发消息时不会自动追问、生成或清除历史。暂缓保留待讨论思路，不强迫用户操作按钮。

核对结果：945项全量后端通过，最后微调后的相关25项、mypy105文件、修改范围Ruff、前端回归和构建通过。真实模型六条意图样例及浏览器暂缓/结束/历史恢复通过，正式服务就绪。郑州三天三人5000样例证实偏好不再被硬条件门槛阻断，但仍有工具输出失败和城市资料不足，未生成结构化草稿；共享资料32份中郑州城市资料0份。完整记录见07本轮验收修正节，不将交互修复写成所有城市规划已验收。

### 12.22 参考方案与运行测量（核对日期：2026-09-21）

普通文字规划的资料不足或结构化输出失败不再只让用户重试。`chat/service.py`将实际限制和旧草稿交给`conversation.py`的planning_fallback提示，按已知人数、预算、偏好给文字参考方案，明确未核实内容；既有草稿不变。沿用原消息保存、等待及自然接续协议，不新增数据库结构。附件专用的必须采用、替换和冲突处理没有统一改成自由建议。

```mermaid
flowchart TD
    HTTP[HTTP请求 / SSE工作线程] --> Metrics[measure_run 请求上下文]
    Metrics --> Workflow[原需求理解与持久工作流]
    Workflow --> Planner[规划与按需研究]
    Planner --> Gateway[原生工具网关]
    Gateway -->|要求工具却只回文字| Retry[现有格式修复与备用]
    Retry --> Planner
    Planner -->|有效候选| Review[程序检查与语义审核]
    Review --> Save[原子保存消息与行程]
    Planner -->|无候选或可降级错误| Chat[自然回答 未核实参考思路]
    Chat --> Message[只保存消息 保留旧行程]
    Save --> Report[汇总metrics]
    Message --> Report
    Report --> Done[SSE done确认结果已保存]
```

**理解与审核：**空检索时提示主规划/研究尽快返回缺口，尚不保证所有重复查询消失。Reviewer不重复推翻已通过的程序时间/算术检查，按语义理解11点等于11:00，第二天要求不套用第一天。已知违反与未核实仍区分；地点来源、明确排除和不变更日保护保持。网关仅把违反tool_choice=required的响应归入原INVALID_OUTPUT，不另加业务审核层；真实网络及鉴权错误仍按原网关处理或报错。

**统计职责与SSE契约：**`chat/metrics.py`用ContextVar隔离一次HTTP执行；`events.py`记录公开过程边界，调用适配层通过`services/usage.py`记录每次外部调用，费用扩展见12.23。SSE退出统计先发metrics，再发done或error；停止仍走原取消终态，统计outcome把取消归入failed。metrics不代表保存成功。普通JSON入口也收集统计并调用INFO日志接口；是否落盘取决于部署日志级别/handler，默认启动未保证该模块INFO日志落盘。前端只读取阶段秒数，保留真实过程文案，不展示内部调用明细。

| metrics字段 | 类型及含义 |
|---|---|
| request_id / outcome | string；请求编号，completed或failed，后者包括取消 |
| total_seconds | number；后端执行范围总秒数，不含完整网络链路 |
| first_draft_seconds | number/null；执行开始到首个非空draft；没有流式正文时null，不是浏览器首包时间 |
| stages_seconds | object；公开stage到累计秒数；相邻事件间隔含阶段内等待，不冒充纯工具耗时 |
| model_calls | object[]；兼容旧字段名，现包括文本、工具模型、视觉、向量、网页、地图和OCR；逐次字段见12.23，耗时不含网关退避 |
| known_total_tokens / unknown_usage_calls | int；已知token之和及用量未知调用数，不能把未知当零费用 |
| cost_cny / usage_scope | string/null / external_api_calls；全部费用已知且无USD金额时才给CNY合计；各币种已知金额另见known_costs |

本节2026-09-21版本的阶段时间只属于运行事件和评测报告。2026-09-22已由12.26更新：成功轮次将公开步骤、阶段耗时和快照耗时保存到response_json.process，刷新可恢复；旧历史缺少过程时不补造。逐次调用另存费用账本，管理员费用页不能替代供应商实际账单。

**固定评测：**`evals/chat.v1.json → scripts/evaluate_chat.py → TestClient真实SSE业务链 → temp/reports`，独立账号会话在结束后清理。六组涵盖模糊需求、资料不足、亲子/指定日修改/异城问题、预算不足、附件冲突、工具故障。故障只在评测进程注入Kimi工具超时，备用真实调用；不修改正式服务健康状态。程序统计明确条件、目标日开始时间及不变更日，正文事实和来源准确率留作人工评阅。报告带固定题库哈希、选定子集、实际回答、失败、耗时与部分用量；TestClient缓冲首事件时间明确标为buffered_first_event_seconds，不当作网络TTFB。

**验收边界：**全量950项通过，随后评分2项及真实已保存响应重评分通过；前端回归和构建通过。真实首轮5/6组通过，失败的亲子组修正提示后单独复测3轮通过，未宣称一次完整6/6。浏览器已看到郑州参考思路和累计阶段时间，仍有重复空查询。详细时间、用量和限制记录在07；大样本来源准确率、P95、全链费用与真实旅行价格仍待完善。

### 12.23 调用费用账本与官方公开票价（核对日期：2026-09-21）

用户确认没有供应商API，本步采用公开官方资料与官方预订入口。完整覆盖的是应用请求中的各类调用记录；现金扣款、套餐折扣、免费额度、税费、汇率、本机硬件电力未对账。公布价不是指定日期可购买报价，单项价格不改写演示预算。

```mermaid
flowchart TD
    HTTP[HTTP请求编号] --> Scope[usage_scope 上下文]
    Scope --> Call[文本/工具/视觉/向量/网页/地图/OCR]
    Call --> Start[先登记 running]
    Start --> Ledger[(usage_events)]
    Call --> Finish[完成或失败 保存用量与费率快照]
    Finish --> Ledger
    Finish --> Metrics[本轮原币统计 metrics]
    Admin[管理员费用页] --> API[GET /admin/usage]
    API --> Ledger
    Planner[原规划及审核] --> Prices[发布前查询官方票价]
    Prices --> Web[Tavily限定官方站点]
    Web --> Extract[本轮所选模型提取公布价及原文]
    Extract --> Save[既有消息与行程事务]
    Save --> Card[行程卡片 价格/适用期/来源/查询时间]
```

**调用与持久化：**`main.py`为HTTP请求绑定保存回调，SSE线程和框架传递的上下文沿用请求编号。`usage_call`在网络调用前登记running，在正常返回、异常或生成器关闭时以同一UUID更新completed/failed；重试分别计费。进程强行退出可能留下running，费用保持未知。`usage_store.py`使用独立短事务，不依赖聊天是否保存成功；写入异常不覆盖业务异常，日志记录请求号/调用号，metrics的unpersisted_calls计数。离线脚本没有usage_scope时不自动写生产账本，旧DeepSeek演示客户端不属于本步在线统计范围；历史调用不猜测回填。

`llm/providers.py`覆盖文本、流式和原生工具，在业务JSON校验前采集用量；`embeddings.py`、`vision.py`分别计数；`web_search.py`记录basic检索；`baidu_mcp.py`在公共工具入口记录直接Agent调用；`attachment/mineru.py`按一次本机解析记录OCR及页数。kind为text/tools/vision/embedding/search/map/ocr。success表示包装范围正常返回，不保证之后的业务解析、保存或外部数据正确。

**计费：**Decimal金额以字符串传输，缓存是输入总token的子集：未缓存输入×单价＋缓存输入×缓存单价＋输出×单价。未知缓存、用量、型号、地域保持amount=null；Embedding只按输入计价。费率版本official-2026-09-21随调用保存，后续代码变更不追改历史。仅匹配已核对的官方型号/端点；北京百炼支持dashscope共享域名及ws-*业务空间域名，其他地区/订阅端点不套北京价。DeepSeek按北京时间时段及2026官方节假日区分费率，未维护的未来年份保持未知。Tavily basic每次1 credit按USD原价估算，未扣免费额度。百度账号费用待对账；本地OCR标local，不记零元API账单。供应商调价需更新代码。

费率依据：[DeepSeek](https://api-docs.deepseek.com/zh-cn/quick_start/pricing/)、[Kimi](https://platform.kimi.com/)、[Qwen3.8 Max](https://help.aliyun.com/zh/model-studio/qwen3-8-max)、[视觉](https://help.aliyun.com/zh/model-studio/qwen3-vl-plus)、[Embedding](https://help.aliyun.com/zh/model-studio/text-embedding-v4)、[百炼端点](https://help.aliyun.com/zh/model-studio/base-url)、[Tavily](https://docs.tavily.com/documentation/api-credits)、[2026节假日](https://www.gov.cn/zhengce/zhengceku/202511/content_7047091.htm)。

**SQL表：**0015_usage_events建立独立账本，前置0014_chat_workflows；0016_chat_workspace再增加可空归属列，正式迁移状态见12.26。字段均无server default，不含正文、密钥或用户/会话外键；删除聊天不删除费用。除主键外没有额外唯一/CHECK约束，JSON结构由写入服务维护。

| usage_events字段 | PostgreSQL类型 | 可空 | 默认 | 含义与约束 |
|---|---|---|---|---|
| id | uuid | 否 | 无 | 主键，调用前生成；同id upsert防止开始/结束重复计费 |
| request_id | varchar | 否 | 无 | HTTP追踪编号；ix_usage_request索引；不是用户或会话编号 |
| user_id | uuid | 是 | 无 | 0016新增；调用账号编号，旧账未知；无外键 |
| session_id | uuid | 是 | 无 | 0016新增；调用会话编号，旧账未知；无外键 |
| message_id | uuid | 是 | 无 | 0016新增；调用消息编号，旧账未知；无外键 |
| started_at | timestamp with time zone | 否 | 无 | UTC调用开始时间；ix_usage_started索引 |
| payload_json | jsonb | 否 | 无 | 调用与费率快照，字段见下表 |

| payload_json字段 | 类型、空值及含义 |
|---|---|
| id,request_id,provider,model,endpoint,kind,started_at | string；非空；endpoint只留hostname，无查询参数 |
| state,success | string/bool；running→completed或failed，开始时false |
| input_tokens,output_tokens,total_tokens,cache_read_tokens | int/null；提供方用量，缺失null，缓存不额外加进总量 |
| units | int/null；网页/地图次数、OCR页数；模型一般null |
| elapsed_seconds,persisted | number/bool；结束补充，耗时与持久结果 |
| error_type,http_status,error_category | 异常时补充：string异常类名、int/null HTTP状态、可选安全ProviderError枚举；旧记录缺省，不回填猜测；不含异常原文 |
| cost.amount,cost.currency | decimal string/null、CNY/USD/null；null表示无法估算 |
| cost.status,reason,rate_version,source | estimated/unknown/local；原因、版本、官方URL/null；不保存异常正文 |
| cost.rates_per_million,cost.period | 已计价模型的input/cached/output字符串费率；DeepSeek的peak/offpeak；其他省略 |

**管理员接口：**`GET /api/v1/admin/usage`执行require_admin，匿名401、普通用户403。start/end必须带时区，范围(0,31天]、默认最近一天；可按request_id筛选，offset>=0、limit=1..200。返回total_calls、known_costs原币字符串合计、status_counts、complete、items、offset、limit、basis。合计独立SQL聚合，不因分页截断；complete仅表示没有未知金额，不表示现金对账完成。UsagePanel提供1/7/30天、请求号、分页和费率依据；普通账号无入口，运行中和未知费用均可见。

metrics新增known_costs、unknown_cost_calls、unpersisted_calls、usage_scope=external_api_calls；model_calls为兼容保留名称，现也含非模型调用。费用独立持久化；阶段耗时在本节初版未入历史，现按12.26随成功轮次保存公开过程快照。API计量不同，网页/地图按次数、OCR按页数，不能将缺token当零费用。

**官方票价：**`travel_prices.py`在工作流publish前查询，失败不阻断行程，不新增等待框或审核Agent。仅搜索gov.cn、lingyinsi.org、dpm.org.cn及子域；片段交给本轮模型理解现行成人基础票价，区分旧价取消、优惠人群及适用期，来源/逐字原文和金额必须对应。取消收费检查仅针对所提金额所在短句，儿童优惠不否定成人价。搜索片段不是对官方服务器的库存查询。每轮最多查询五个地点/日期组合；仅同城市、同地点、同日期的成功公布价15分钟内复用，换城市及失败/过时缓存重查。历史版本保留原查询时间。不能从静态沙盒price推断免费，不保证覆盖所有景区。

**行程JSON：**`TravelPlan.ticket_prices`数组默认[]，兼容旧快照；沿用itineraries/requirement_turns及检查点既有JSON，无新增价格表、外键或索引，不改变预算算术。

| ticket_prices元素字段 | 类型、空值、默认及含义 |
|---|---|
| place | string，必填；本次地点 |
| travel_date | date/null，默认null；出行日 |
| status | published/unavailable/unconfigured/error/expired/not_applicable；必填；后两项为已过期/尚未生效 |
| amount,currency,unit | decimal string/null且>=0(默认null)、CNY、string默认空；基础公布价与单位 |
| conditions,evidence | string默认空；条件和逐字原文 |
| source_url,source_title,published_date | string/null，默认null；官方网址、标题、发布日期原值 |
| checked_at | UTC datetime，程序生成；不等于生效日期 |
| valid_from,valid_until | date/null，默认null；原文明确给出才提取 |
| date_confirmed | false，固定false；未确认指定日期库存或可购买 |

ItineraryCard显示公布价、适用期、原文、链接和查询时间；过期/未生效不显示为所选日期的可用价。demo-cny-v1预算仍为演示估算，单项成人价不自动乘全团或替换总预算。12306、国航、华住会官方入口供用户查询指定日期价格；没有代订、私有库存抓取、支付或预订接口。

**验收：**最终全量966项、0跳过，mypy112文件、修改范围Ruff、前端构建/回归通过。真实行程18次调用均持久保存、费用与metrics一致；浏览器费用筛选及行程票价历史恢复通过。独立审查的跨城市同名缓存和优惠人群误拦已修复并回归。正式迁移0015；费用真实扣款、全景区覆盖及库存报价未验收为可用能力，详见07。

### 12.24 模型降级诊断（2026-09-21核对）

调用失败→usage_call保存异常类名/HTTP状态→提供方适配器归类→网关按原策略发出fallback→前端过程区显示具体原因。HTTP 429表示请求频率或token吞吐受限，不能写成余额不足；未知类别仍用通用提示。只显示程序定义的错误类别，不透出供应商异常原文。本节初版的“过程仅本页保留”已由12.26替代：新成功轮次保存公开步骤，旧历史仍不补造失败原因；后台账本独立保留安全诊断字段。本节诊断字段本身未新增SQL结构，后续工作区迁移另见12.26。

显式选模仍优先本轮active或用户selected，失败后依次尝试DeepSeek/Kimi/Qwen并去重；选模模式下瞬时故障优先快速切备用，同轮后续请求继续使用备用。一次对话包含多次模型调用，账户余额与RPM/TPM限额是不同约束。真实复现Kimi账户RPM=3导致第四次请求429；Qwen基础工具、独立规划和原历史上下文复查均成功，不能推定旧失败同样是限流。新增诊断只覆盖上线后的调用，旧记录不回填。相关117项后端测试及前端模型选择回归/构建通过；平台限额未由项目改变，详细证据见07。

### 12.25 自动交通查询（2026-09-22核对）

明确交通查询复用本轮理解模型产生transport_query，不派发规划或地图Agent。交通支路单独保留WebSearchClient，普通聊天与原规划仍不自动开启网页检索。近期transport.query和实际当前日期随历史上下文传入，查询城市与旅行目的地分开，交通结果不会生成新行程版本。旧记录transport=null兼容读取。

```mermaid
flowchart LR
  U[用户与历史条件] --> T[统一理解 transport_query]
  T --> Q[交通查询服务]
  Q --> D[原出发日 + 可选前夜 + 返程独立查询]
  D --> R[12306公开站名/余票/票价]
  Q --> F[Tavily限定航司官网]
  R --> S[往返独立筛选与成人参考金额]
  F --> S
...
`RailClient`使用独立httpx连接，初始化页面确定当前站名资源和余票入口；入口只允许固定12306域名及限定路径，不跟随任意外链。余票列位置参照官方页面JS，票价读取官方O字段（人民币成人二等座），乘人数由Decimal计算。15天含当天以北京时间判断；每程最多三班按出发时间排序。保留官方实际站名，不把郑州西/东合并成同站；无中转搜索。查询前后检查停止，网络调用8秒超时；拒绝访问和结构变化显示unavailable，不绕过验证，也不说售罄。price失败保持null，保留可用班次。

“前一天晚上也考虑”保留原departure_date，另查前一天；无论前夜found、empty或unavailable都继续原出发日和返程，且前夜不增加航司检索。前夜最晚到达上限锚定原出发日，允许跨夜抵达。交通缺项补齐优先接续查询；预算包含城际车票的补充优先取得交通事实，不反复返回逐日规划。自然回答沿用已确认的全团预算，缺出发地/日期时先简短给方向与补问；不从未知票价计算剩余预算，不用“约XX元待核实”变相提供未经核实票价。铁路Markdown拆分车次、车站、时刻、耗时、单价、成人小计及余票，Vue通过深层选择器使v-html表格的间距、表头和横向滚动实际生效。

航司检索域名限定国航、东航、南航、海航、厦航、深航、川航、山航、春秋、吉祥官方域名；每程一次检索，复用WebSearchResult原文、URL、发布时间及查询时间。found只说明找到网页，不表示同日库存。交通支路不再额外调用回答模型：状态、筛选条件、铁路表格、金额乘算、已知余票不足人数提示及查询来源直接由程序输出，避免模型重报事实或推测飞行耗时。过程区显示实际查询步骤，查询失败仍保留已完成的其他结果。

**JSON契约（沿用requirement_turns.response_json和工作流检查点，无SQL字段、索引、外键或迁移变化）：**

| 字段 | 类型、默认与含义 |
|---|---|
| TurnUnderstanding.transport_query | TransportQuery/null，默认null；明确交通查询时填写，response_mode=chat |
| TurnUnderstanding.transport_continue,transport_clear_fields | bool默认false、允许清除的交通字段枚举数组默认[]；明确接续才合并旧查询，未提及/null保留，明确解除才清空；独立新查询不继承 |
| response.transport | TransportResult/null，默认null；本轮不可变查询快照 |
| query.origin,destination | string/null，默认null，最长80；当前查询城市/站名 |
| query.departure_date,return_date | date/null，默认null；去/返日期，返程空表示单程 |
| query.travelers | int/null，默认null，1..100；仅用于成人参考小计，不推断优惠资格 |
| query.modes | rail/flight数组，默认两项，长度1..2 |
| query.earliest_departure,latest_arrival | time/null，默认null；仅去程硬筛选，空表示不限 |
| query.previous_day_earliest_departure | time/null，默认null；额外考虑原出发日前一天此时刻之后的铁路，不替换departure_date，不作用于原出发日或返程；可通过transport_clear_fields明确取消 |
| query.return_earliest_departure,return_latest_arrival | time/null，默认null；仅返程，绝不回退去程字段 |
| query.preferences | string默认空，最长300；不确定的软偏好，不自动改成硬时段 |
| transport.missing,legs | string[]/TransportLeg[]，默认[]；缺少城市/日期时不要求预算 |
| leg.origin,destination,date | 必填string/string/date；回程交换两城市 |
| leg.rail,flights | RailResult/null默认null；WebSearchResult默认not_requested |
| rail.status | found/empty/unavailable/not_on_sale/past_date/unknown_station；empty仅本次筛选无直达候选 |
| rail.checked_at,source_url,note,options | UTC datetime程序生成、官方查询链接、说明默认空、候选默认[] |
| option.code,from_station,to_station,seats | 必填string；官方班次/实际站名/余票原值，非锁座承诺 |
| option.departure,arrival,duration_minutes | 必填带时区datetime/datetime/int；到达由官方时长加出发计算并比对官方钟点 |
| option.price | Decimal/null，默认null；单程人民币成人二等座，不含接驳或优惠 |

费用仍沿用0015：12306公开读取记provider=12306、kind=transport、units=1、endpoint仅hostname，不保存订票令牌或原始余票响应；当前无网站调用费率，不假定为实际零扣费。旅行预算仍为演示费率；交通快照不自动更改预算。源码、自动化和真实SSE验证范围见07，航班完整库存与可购买报价未交付。


### 12.26 对话工作区生产接入（核对日期：2026-09-22）

**当前边界：**已确认的`designs/travelmind-workspace-v2`用于布局参考，生产页面读取真实接口。新增工作区状态、个人文件与知识库检索测试入口；普通聊天、规划、附件与共享资料仍走原业务服务。此节取代此前“过程只在本页”“标题仅截原话”“工作区未增加字段”的旧阶段描述，不追改旧历史，也不新增自主智能体。

```mermaid
flowchart TD
    UI[App 对话工作区] --> Chat[原消息 HTTP / SSE]
    Chat --> Owner[登录与会话归属校验]
    Owner --> Scope[账号 / 会话 / 消息调用范围]
    Scope --> Business[原聊天 / 规划 / 审核业务]
    Business --> Tools[真实资料读取 / 知识库 / 网页 / 地图调用]
    Business --> Research[实际委派研究子智能体]
    Research --> Events
    Tools --> Events[call_id + 状态 + 耗时 + 安全摘要]
    Events --> Stream[SSE公开过程]
    Stream --> Groups[四类工具 + 子智能体分组摘要]
    Events --> Save[原事务保存回答与process快照]
    Save --> Turns[(requirement_turns)]
    Save --> Title[首轮短标题 manual优先]
    Title --> Sessions[(sessions.title / title_source)]
    Scope --> Ledger[(独立usage_events账本)]
    UI --> Popover[原生状态浮层 不占聊天宽度]
    Popover --> Status[workspace-status 仅会话所有者]
    Status --> Ledger
    UI --> Summary[聊天中的每日行程概览]
    Summary -->|保存版本的itinerary_id| Files[personal-files 仅本人]
    UI --> Files
    Files --> Existing[(私人附件 / 行程 / 需求 / 会话)]
    UI --> KB[管理员文件管理 / 检索测试]
    KB --> Search[共享资料原接口 + 混合检索]
    Search --> PG[(PostgreSQL原文)]
    Search --> MV[(Milvus向量)]
```

**页面与调用职责：**`App.vue/chat.css`组织左侧历史、中间聊天、独立状态浮层和右侧文件；`ThinkingProcess`将读取资料、检索知识库、联网搜索、地图查询合并为每类一行，实际子智能体委派单列；普通准备阶段不逐条展示。摘要显示调用数和真实状态，按需展开具体记录，明细区高度上限220px；未使用的类别不补造。`WorkspaceStatus`读取实际统计，界面仅保留最近上下文占用与累计Token/缓存用量，移除模型与服务和最近调用区块；统计接口继续保持原合同。状态使用原生popover，在右上角悬浮，点击外部或Escape关闭，不占聊天宽度；与文件同时打开时Escape只关闭最上层状态。`PersonalFilesPanel`同时服务个人空间与当前会话侧栏。左侧导航直接采用设计稿的文字历史列表、新建气泡加号和铅笔入口，移除旧聊天图标及重复导航样式；铅笔直接改名，右键或Shift+F10保留其他操作，不删除对话数据。`ItineraryCard`在聊天中使用compact概览，仅展示标题、天数/人数/日期、每日路线及文件入口；文件预览继续使用完整卡片。文件入口按该条消息的itinerary_id读取对应历史版本，迟到请求不能覆盖新选择；Markdown下载保留活动、地址/坐标、路线估时、来源正文、预算明细及公开票价，不只导出概览。规划未生成结构化快照时仍保留真实文字建议，不能伪造已保存文件。`DocumentPanel/DocumentSearch`组织管理员文件与检索测试。文件侧栏默认360px，最小300px，上界为760px与当前工作区剩余空间共同限制；指针、方向键（20px，Shift为50px）、Home/End、双击复位共用边界，localStorage仅保存宽度偏好。文件侧栏在工作区宽度小于900px时使用带焦点限制、Escape关闭及焦点返回的抽屉。

**新增及扩展HTTP合同：**路径均加`/api/v1`，账号取登录Cookie，不能由客户端传user_id；管理员不获得其他用户私人会话/文件的读取权。

| 接口 | 输入与返回 | 权限和含义 |
|---|---|---|
| `GET /sessions` | 新增可选q最多200字；limit=1..100默认30，保留cursor；返回items/next_cursor | 本人会话标题按普通文本匹配，百分号与下划线不当通配符；筛选在SQL分页前执行 |
| `GET /sessions/{id}/workspace-status` | 返回累计、按型号汇总、最新上下文及最近调用，字段见下表 | 先require_session_owner，缺失/他人404；累计独立SQL聚合，不受最近20条展示上限影响 |
| `GET /personal-files` | 可选session_id、kind=attachment/itinerary、q最多200字；offset>=0、limit=1..100默认30；返回items/total/offset/limit | 本人私人附件与每个会话最新daily-plan-v1行程；指定他人会话404；总数按完整相同筛选条件计算 |
| `GET /personal-files/{kind}/{identifier}` | 返回item、attachment或itinerary、extraction、versions | 每次按文件UUID重新核对本人归属，行程历史版本使用各自需求快照；未知或他人404 |
| `GET /personal-files/itinerary/{identifier}/download` | 所选已保存版本即时生成UTF-8 Markdown；Content-Disposition下载文件名 | 不在磁盘存重复文件，来源会话、版本、需求、活动、证据及预算估算随文件输出 |
| `POST /admin/document-search` | 沿用query=1..800字、limit=1..10默认5、document_id=null或UUID、city/category；返回items(score/file_name/chunk) | 管理员共享检索；混合排序分不是相似度或内容正确率，不新增搜索后端或模拟命中 |

个人文件条目包括UUID id/session_id、kind、file_name、mime_type、session_title、created_at以及可空size_bytes/version/version_count。行程未生成下载内容时size_bytes=null，页面显示Markdown；不根据文本猜大小。versions只列同一会话已保存的逐日行程；早期无聊天快照的版本可重建展示快照，不提供伪造的撤销操作。附件下载沿用原`/sessions/{session_id}/attachments/{id}/content`及其归属校验。文件列表、详情和下载设置no-store；原文与行程正文不进入公共知识库。

**工作区状态字段与计算：**

| 字段 | 类型及口径 |
|---|---|
| total_calls | int；当前账号、会话全部账目数量，包括失败或运行中调用 |
| known_total_tokens / unknown_usage_calls | int；已知总Token之和与缺失用量调用数，未知不当零消耗 |
| cache_hit_ratio | number/null；text/tools/vision范围的缓存读取Token之和除以输入Token之和；任一相关输入/缓存未知或无有效分母时null；地图/OCR/Embedding不污染分母 |
| latest_context / contexts | object/null与object[]；每个真实provider/model最近一次purpose=conversation且kind=text/tools的调用；标题和历史摘要不冒充聊天窗口 |
| contexts各元素 | provider/model/purpose/started_at，input_tokens=null或int，context_window=null或正整数，ratio=null或input_tokens/context_window；最近输入未知时不回退旧值 |
| models | 每个provider/model的calls、可空input_tokens/output_tokens/cache_read_tokens及unknown_usage_calls；按全部会话调用分组 |
| recent_calls | 最近20条id/provider/model/kind/purpose/state/started_at、可空输入输出Token与elapsed_seconds；不返回原始提示词、密钥或完整工具参数 |
| unattributed_history | bool；有已保存消息缺少对应归属账目时true；旧账不猜测回填 |

`config.py`的model_context_windows默认空映射，只接受正整数。`backend/.env.example`列出`deepseek-flash`和`deepseek-v4.1-flash`为1048576；`qwen3.8-max`、`qwen3.8-max-0902`、`qwen3.8-max-2026-09-02`为1000000；`kimi-k2.6`为262144（256K）。查不到实际型号、容量或输入Token时保持null，累计用量不能作为窗口占用。2026-09-22容量依据：[DeepSeek型号说明](https://api-docs.deepseek.com/quick_start/pricing/)、[DeepSeek精确窗口配置](https://api-docs.deepseek.com/quick_start/agent_integrations/codex/)、[Qwen3.8 Max](https://help.aliyun.com/zh/model-studio/qwen3-8-max)、[Kimi平台](https://platform.kimi.com/)。这里只核对容量，不扩展费用或改变输入裁剪。 状态文案为“最近调用上下文”，并列实际输入/容量。实际管理员旧会话6037/1048576=0.576%，显示0.6%；108669是累计输入输出消耗，不能当作当前占用。应用`llm/budget.py`另有64000字符的输入预算，并按需处理历史摘要，这与模型Token容量不是同一单位，也未在本次修改。

**工具事件与历史JSON：**`events.py`在实际执行边界记录call_id，开始为running，结束为completed/failed/cancelled；可用正常响应表达的地图业务错误也标failed。`itinerary/research.py`在实际子循环边界复用tool_progress，stage=subagent，名称为研究助手，结束区分完成、未返回结果（failed）、异常及取消；不会公开任务原文或内部推理。旧research_start/research_done只在前端配对展示，不回写历史；有结束事件只显示已返回，不推断研究成功，没有结束事件保留未完成。子智能体次数与普通工具次数分别显示。没有调用的步骤不补造。`metrics.py`按call_id更新同一条记录，`RequirementHistoryService.append`把保存前取得的快照写入既有`requirement_turns.response_json.process`；`useRequirementConversation`恢复它，旧历史默认null。

| process字段 | 类型、可空、默认与含义 |
|---|---|
| process | object/null，默认null；只在成功轮次保存公开快照 |
| steps | object[]，默认[]；公开步骤，按实际发生顺序 |
| steps[].stage / message | string，必填；程序定义的阶段与安全说明 |
| steps[].call_id | string/null，默认null；真实工具或子智能体调用配对编号，普通阶段可空 |
| steps[].status | running/completed/failed/cancelled/null，默认null |
| steps[].elapsed_seconds / summary | number/null与string/null，默认null；真实工具执行时间与安全结果摘要 |
| seconds | number，必填；运行开始到获取快照的秒数，不等于网络往返总时间 |
| timings | object，默认{}；各公开阶段累计秒数，包含阶段内等待，不冒充独立工具耗时 |

SSE metrics仍在执行范围退出时发出，done才确认已保存。标题生成发生在整轮保存之后，因此成功历史的process快照不包含后续标题耗时；标题调用进入独立账本，purpose=title；历史压缩purpose=summary，其他会话调用默认conversation。失败/停止不会新增成功过程快照，在当前页面保留中断状态；原调用账目仍独立保存。此JSON扩展未新增过程SQL表或列，不记录内部推理、异常正文和原始工具参数。

**标题状态与事务：**默认“新建对话/旅行需求对话”创建为pending，其他指定名称为manual。首轮原事务保存原话去空白前40字（纯附件可取文件名）作为回退值；整轮完成后`session_titles.py`原子认领pending→generating，释放事务再调用本轮模型。输出必须为1..40字JSON标题；成功改auto，失败改fallback并保留回退标题。最终更新必须仍处于generating，手动PATCH始终写manual，所以模型迟到不会覆盖用户名称。仅首轮revision=1触发，旧会话迁移为manual，不重跑历史模型；进程在generating中退出不承诺自动恢复标题任务。

**0016迁移与逐字段增量字典：**`0016_chat_workspace`前置`0015_usage_events`，正式服务已迁移0016；没有新业务表。既有主键、会话外键、唯一约束保留，新增项如下；sessions完整字典见7节，账本完整字典见12.23。

| 表.字段 | PostgreSQL类型 | 可空 | 数据库默认 | 含义与约束 |
|---|---|---|---|---|
| sessions.title_source | varchar(20) | 否 | manual | 标题来源；`ck_sessions_title_source`检查pending/generating/auto/manual/fallback |
| usage_events.user_id | uuid | 是 | 无 | 调用所属账号；旧账null；无外键 |
| usage_events.session_id | uuid | 是 | 无 | 调用所属会话；旧账null；无外键 |
| usage_events.message_id | uuid | 是 | 无 | 调用所属消息；旧账null；无外键 |

新增普通组合索引`ix_usage_owner_session(user_id, session_id, started_at)`；已有`ix_usage_started`和`ix_usage_request`保留，没有新增唯一约束。usage_events仍以id为主键，开始与结束按同id更新payload，不重复计费。新归属列与users/sessions/message_id仅为逻辑关联，**故意没有外键或级联删除**：删除会话后费用审计继续保留，原会话状态接口因会话已不存在返回404。独立账本的payload同步保留可空归属与purpose，历史没有的信息不补造。个人文件复用conversation_attachments、itineraries、travel_requests、requirement_turns与sessions，未新增文件表。

**验证分层：**本轮全量980 passed、2 skipped，跳过的两个Milvus测试当时未配置测试地址；随后定向40项通过并补跑这两项真实Milvus，不能将后者改写成又跑了一次全量。mypy120文件、前端6个回归脚本及生产构建通过。新增定向覆盖权限、标题竞争与普通文本搜索、原生tools上下文、未知最新输入、缓存统计过滤和地图业务错误事件；正式服务PostgreSQL/Milvus均ready且就绪HTTP 200。

真实后端已用DeepSeek完成苏州两天新建v1与修改v2，自动标题为“上海出发苏州两日游行程草稿”；首轮账目30次调用、已知63559 Token，最新输入4128/1048576约0.4%，与累计消耗分开。专用账号上传TXT后当前会话文件2条，v1/v2 Markdown下载均200；管理员搜索“苏州园林”limit=3返回3条同源片段，跨读专用账号文件及状态均404。这些是实际API证据，不代替前端操作验收。

已完成浏览器桌面1440与手机390宽布局检查：侧栏360→拖动506→键盘526、刷新保留、双击恢复360，手机Escape关闭，无水平溢出。个人空间表格、当前会话文件卡片、TXT正文预览、放大收起、v2切换v1及来源会话跳转均通过；管理员知识库筛选、检索和原文查看通过，普通账号无管理入口。第三轮真实知识库问答保持原行程版本，保存的检索片段数及来源摘要可在刷新后展开。最终控制台无error/warn。专用验收账号与会话已清理，真实调用账本按约定保留。

`PersonalFilePreview.vue`复用逐版本行程快照，供个人空间与当前会话文件区预览、放大及收起；文本按纯文本展示，PDF遵循现有安全策略打开原件。标题生成的数据库认领或保存错误不影响已经保存的回复；最后标题及工作区定向7项检查通过。

### 12.27 浏览器自动回归边界（2026-09-22核对）

M5已按三模型和体验优先范围收口：文本/JSON/原生工具共用选择与故障策略，主规划与研究上下文共享首选，已有结构校验、有限修复和保存恢复。GatewayState保留进程内成功率、P95和fallback次数，管理员统计包含原生工具；不宣称多进程共享健康或旅行端到端P95已达标。历史12.18/12.19的待实现描述由12.20及后续实现取代。

M6新增Playwright开发期浏览器层，生产架构、接口及数据库结构不变：

```mermaid
flowchart LR
    P[Playwright 桌面与手机] --> V[4175 构建产物预览]
    V --> U[真实 Vue 页面与 fetch/SSE 处理]
    U --> F[每例独立固定接口夹具]
    F --> H[内存会话 行程 文件 用量]
    U --> A[原生浮层 DOM 布局和键盘断言]
```

`playwright.config.ts`自动启动并结束预览，拒绝复用占用端口；`workspace-fixture.ts`拦截全部API，未知请求不转发正式后端，页面脚本异常也导致失败。`workspace.spec.ts`验证首选模型发送/恢复、过程分组与子智能体、聊天摘要/完整文件、悬浮状态比例、Escape、手机宽度以及409后读取新历史并以新revision重发。`tsconfig.browser.json`让夹具复用生产DTO且独立类型检查。失败trace位于根目录temp/runtime；没有新增SQL字段或持久表。

本次后端992项全量、0跳过（隔离PostgreSQL/Milvus），mypy120文件、原前端6组脚本和构建通过；新增浏览器桌面/手机共6项通过。浏览器接口模拟与此前真实管理员服务验收分层记录，尚不能替代Compose全栈自动验收。M6继续进行，M7继续承担广泛质量和新环境部署验证。

### 12.28 真实服务浏览器验收入口（2026-09-22核对）

新增`scripts/evaluate_browser.py --live`与`playwright.live.config.ts`，保留12.27固定接口测试作为快速回归。入口先检查测试类型、读取backend/.env，建立随机专用账号并通过本机8000登录核对数据库身份，然后把仅用于测试的账号口令放入子进程环境，启动4176构建预览；不会启动第二个共享后台或恢复正式任务。

```mermaid
flowchart LR
    R[Python验收入口] --> U[建立专用普通与管理员账号]
    U --> P[Playwright真实页面 4176]
    P --> A[现有FastAPI 8000 HTTP与SSE]
    A --> G[真实模型网关 规划与工具]
    A --> D[PostgreSQL 持久版本与文件]
    P --> C[读回核对与实际文件下载]
    C --> E[成功清理本次账号与会话]
    E --> F[调用账目独立保留]
```

真实模式不拦截API、不自动重试模型调用、不保存登录trace；密码框失败只抛安全错误。失败可能有在途请求，保留专用账号编号，确认结束后按归属清理，不按标题批量删除。清理复用TripService/Auth模型，独立UsageEvent无级联删除；生产模块与表结构不变。

页面在SSE done后会取消reader，因此测试不依赖Chromium调试接口回读整条流；等待界面解锁并用真实历史接口核对最后原话与新版本。固定接口/真实模式分别构建至temp/runtime/m6-browser-dist、m6-live-dist，避免并行构建短暂清空另一套页面。

实测真实DeepSeek两轮v1→v2、第一天和预算保持、第二天11:00、刷新、v1/v2文件及页面下载、双角色私人内容拒绝、真实409不重复保存通过（该小步最终38.0秒用例）。固定接口6项、独立构建首页/脚本200、新增清理边界1项、类型/静态检查通过。该小步当时尚未建立全应用Compose；后续上传及新环境结果见12.29，不将一条真实主路径扩写为M6全部完成。

### 12.29 上传验收与独立全应用Compose（2026-09-22核对）

```mermaid
flowchart LR
    B[浏览器 localhost:8080] --> N[Nginx Vue静态页面]
    N -->|同源api Cookie SSE不缓冲| A[FastAPI 单进程 非root]
    M[一次性Alembic迁移] --> P[独立PostgreSQL]
    A --> P
    A --> V[内部milvus:19530]
    V --> E[独立etcd]
    A --> U[uploads命名卷]
    A --> L[backend.env配置的模型与地图服务]
    A -. PDF 图片 OCR .-> H[独立宿主机MinerU:8010]
    T[真实浏览器验收入口] --> B
    T -->|仅本机15432 建专用账号| P
```

**部署合同：**项目名`travelmind-app`，PostgreSQL、etcd、Milvus、uploads均为独立卷，不读取原开发数据库或向量卷。迁移等待PostgreSQL就绪，API等待迁移成功和Milvus健康，网页等待API健康。新库实际迁移到0016；表结构和字段字典不变，没有新增迁移。API与迁移只读挂载backend/.env，环境变量覆盖容器数据库、Milvus和原件路径；配置不进镜像。`MilvusStore`只增加内部服务名`milvus`，仍拒绝其他远程域名和地址中的凭据/查询/片段。8080网页、15432数据库只绑定127.0.0.1；Milvus/API不向主机发布端口。

**HTTP与运行边界：**Nginx保留含端口的Host供Origin核对，Cookie仍`Path=/api/v1`；SSE禁缓冲、900秒读取超时，上传代理32MiB，业务层仍分别校验原格式限额。API以UID10001运行，uploads卷承载原件；模型网关/停止状态仍是单进程。本套为本机HTTP，不代表公网TLS或集群部署。MinerU独立运行，Compose指向host.docker.internal:8010；本轮宿主机该端口未监听，OCR未验收。Cookie不按端口隔离，用户同时使用两套系统时应分主机名或浏览器配置。

**真实验收调用链：**`evaluate_browser --live --compose`读取正式.env并派生本机独立库地址，随机创建两个普通账号和一个管理员，通过同源登录核对ID后将短期测试配置传给Playwright。测试不拦截API，不自动重跑收费请求。预检每次登录前清空客户端Cookie，保留各自令牌用于结束后的远端会话删除，避免账号切换撤销前一个令牌和重复登录限流；容器API负责删卷内原件，再删账号，usage_events不级联删除。失败或清理异常报告精确UUID，确认没有在途任务后才能清理。

私人TXT/MD走现有解析→模型结构校验→不可变消息快照，不进入共享向量库；共享上传走管理员接口→后台任务→SQL片段→Milvus→聊天来源，最后按新建资料ID删除并核对搜索。规划测试自建唯一最小参考资料，避免空库依赖开发者历史数据。`support.ts`共享登录/SSE回读，`journey.spec.ts`负责版本与权限，`uploads.spec.ts`负责上传/引用/清理，生产接口没有新增字段。

**资料问答边界修复：**当没有真实附件、没有规划意图/模式，且模型仅标read/reference、apply_to_plan=false时，忽略错误附件用途并把原问题补为检索查询。明确缺件规划/替换仍走补问保护；不会把other标签统一改成旅行。理解提示要求已保存资料/活动通知先检索，未检索不能断言没有资料；available_attachments为空时不能只凭“资料”一词走私人附件。

**运行证据：**独立新库中规划两轮及文件/权限/409通过；最终TXT/MD上传组2项通过并完成清理；整套重启后的Cookie、会话、原件哈希及向量查询通过。最后一次全套曾2通过1失败，追加提示修正后只定向复跑上传组，未写成全套一次通过。附件结构输出曾偶发失败，后续诊断与运行通过但无确定根因；OCR、缺项接续、故障切换的Compose浏览器场景仍待覆盖，不能以此宣称稳定性或M6全部达标。详细失败及分层检查数见07最新记录。

### 12.30 OCR、结构修复与指定日附件接续（2026-09-23核对）

12.29描述的是当时的运行状态。本次宿主机MinerU已启动并由容器通过`host.docker.internal:8010`健康检查；PDF、PNG的浏览器原件走`AttachmentReader`→MinerU完整解析→文字分析→私人`conversation_attachments.parsed_json`和本轮`response_json.attachments`，刷新直接恢复不可变快照。`parser_version`为现有后端字段，前端DTO补齐类型，没有迁移或新增HTTP字段。未配置百炼视觉时，本次纯文字图像由MinerU OCR读取；复杂图表的视觉补读仍需视觉配置，不将纯文字样本扩大为任意图片成功率。

文字附件和MinerU正文批次共享`analyze_text_json`：首次模型JSON若字段校验失败，向同一模型提交不含原始输入值的字段位置/原因，最多修复一次；仍失败则保存识别失败状态和可重试原件。格式修复不跳过正文地名、页码和引文校验，也不把模型输出写入公共知识库。图片/PDF视觉补读仍沿用原校验路径。本次变化只涉及附件文字识别服务及其调用次数，无账本或数据库结构改变。

一次真实指定日修改暴露两个误解：旧11:00硬条件未被撤销，以及把同一留园的开始时间改动标成新地点替换。`build_understanding_prompt`现在明确用户“改为”撤销同日旧时间限制；同地点时间调整用`reference`加目标日，只有新地点替换旧地点才用`replace`。后端仍以原`resolve_attachment_use`和`validate_attachment_plan`核对目标日、地点依据与不变日，不放松替换校验。失败轮次仅保存补问，旧v2草稿不覆盖；后续真实v3成功保持第一天及预算。该模型理解规则有真实小样本证据，不能等同确定性语义保证。

`evaluate_browser --live --compose --scenario all`运行普通行程、上传、缺项补问和OCR五项浏览器用例；故障注入仅以`--scenario failover`在独立Compose临时配置不可连接Kimi地址，实际备用DeepSeek，随后恢复原API配置。所有用例均由前端真实fetch/SSE进入独立PostgreSQL/Milvus和现有模型，成功后按账号删除会话及原件，`usage_events`保留。部署仍是本机HTTP单进程，外部新机器验收与大样本稳定性归M7；分次及最终验收数见07当天记录。
