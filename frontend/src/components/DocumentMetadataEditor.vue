...
import { updateDocumentMetadata, type DocumentDetail, type DocumentMetadata } from '../api/documents'
...
watch(() => props.document.id, () => {
  ...
  metadata.value = { city: value.city, category: value.category }
  message.value = ''
  saved.value = false
}, { immediate: true })
...
async function save(): Promise<void> {
  ...
  saved.value = false
  try {
    const result = await updateDocumentMetadata(props.document.id, metadata.value)
    if (active && props.document.id === result.id) {
      metadata.value = { city: result.city, category: result.category }
      saved.value = true
...
<template>
  <form @submit.prevent="save()">
    ...
    <small>无法确定的城市或类别保留空值。修改标签无需重建索引。</small>
