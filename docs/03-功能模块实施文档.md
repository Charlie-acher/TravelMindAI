...
目录统一按 [项目分层约定](06-项目分层与审阅.md) 执行：`backend/app` 为正式包，API 不设版本号文件夹，业务规则在 services，模型调用在 llm，工作流在 agent，检索在 rag。
...
2026-09-10实施调整：按用户要求，Vue 3 + TypeScript + Vite + Arco Design的简易对话页提前到M2，直接调用真实后端验收。当前已完成单条抽取、多轮合并、对话HTTP和浏览器交互；下一步消息保存/恢复及30条数据集验收。逐文件说明持续覆盖更新 [07-各阶段开发说明](./07-各阶段开发说明.md)，不再单独增加08、09文档。
