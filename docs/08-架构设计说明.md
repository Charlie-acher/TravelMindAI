...
> 最近源码核对：2026-09-18（Asia/Shanghai，D私人附件、百炼Qwen视觉读取与消息快照；附件修改行程仍待M4-B）
...
TravelMindAI 当前采用 **Vue 前端 + FastAPI 后端 + DeepSeek需求理解与证据回答 + create_agent周边查询与文本行程（底层LangGraph） + PostgreSQL正文和版本快照 + 阿里云Embedding与Milvus语义搜索**。
...
flowchart TD
  ...
  C -->|知识库、网页、地图| T[ToolNode顺序执行，外部查询共用12次额度]
  T -->|原生ToolMessage| C
...
**工具与证据：**`knowledge_search(query)`复用共享范围的DocumentSearchService，最多取5条原文；`web_search(query)`复用已有网页客户端；`map_lookup(name, source_ids)`只能查询已返回原文中实际出现的地点名，查询限制当前目的地。最终活动只能引用本轮或同目的地旧行程保存的place.id，新核对地点必须是百度found、match_kind=poi且有BD-09坐标的地点；不把行政中心、模糊匹配或未核实名称放进草稿。工具没有知识库管理、文件读写或账号操作权限。
...
以上是特定样例证据，不是时延承诺、全部商户召回或逐句正确保证。单独路线问答可用，不代表逐日行程自动逐段核实路线、开放时间、票价及可达性；D私人附件后续已落地见12.5，附件修改行程、独立Reviewer和持久恢复仍未实现。
...
核对日期：2026-09-18。账号、会话归属和共享知识库字段见第7.5节；M4-A文本行程与撤销见10.23，D附件已落地字段和本轮验收见12.5。文件替换及M4-B/C继续作为目标，不将识别结果等同于行程修改。产品和交互详情见[09方案](./09-双角色与对话附件改造方案.md)。
...
| 用户附件（D已实现） | 当前用户与会话校验 → 原件保存 → 文字/图像理解 → 识别快照；地点核对与草稿修改待M4-B | 附件业务和视觉适配器；文字读取复用parser，不调用chunker/向量入库 |
...
第2节三张静态图保留M1～M3基础，2.5及10.23展示当前M4-A源码；D当前数据流见12.5，M4-B/C仍是目标。
...
主键id；storage_name唯一；(session_id, content_hash)唯一，只在同会话去重；(session_id, created_at)索引用于读取，查询另以id稳定排序。session_id外键sessions.id，默认限制删除。迁移0011_conversation_attachments接0010；本机已应用。没有processing、updated_at和处理超时索引：当前同步识别，成功/失败才写状态，进程中断仍可重新发送。
...
附件表不放message_id外键，上传不调用模型；请求SavedRequirementMessage.attachment_ids默认为[]、最多3个不同UUID。响应response_json.attachments默认[]，保存{id,file_name,size_bytes,analysis,error_message}快照；size_bytes为可空正整数，默认null，旧历史自动兼容，不补跑模型；这是JSON快照字段扩展，不新增表列或迁移。保存时再次检查全部id属于当前会话。相同message_id但文字、前置版本或附件列表不同返回409；已提交重试返回原快照。识别结果可以先缓存到附件表，消息冲突不会修改旅行条件、行程或已有消息；并发未提交请求仍可能重复调用模型，不承诺恰好一次模型执行。
...
不新增messages或第二套trips/days主表。M4-A在requirement_turns.response_json.itinerary内保存行程编号、版本和计划，见10.23；D已增加请求attachment_ids和响应attachments识别快照，见12.5，旧快照按空列表兼容。附件用途与目标日待M4-B扩展。子字段由Pydantic和业务检查，JSON中的行程编号不是新的数据库外键；itineraries已有(request_id, session_id)复合外键继续保证草稿与需求同会话。
...
0008账号与过渡会话归属、0009城市/类别/上传管理员、0010强制会话归属已在本机真实库执行，此前备份记录为`backend/data/runtime/backups/m3-roles-20260916-095334`；清理及范围迁移数量见12.2与12.6。2026-09-18另备份public后应用D的0011附件表，Alembic check无差异，见12.5。M4-A复用既有JSONB；文件替换F及M4-B/C仍待实现。
...
**附件用途与撤销：**M4-A已保存修改前后行程版本、实际差异摘要和撤销幂等依据，实际字段见10.23。不为本轮意图单独建表；附件用途（reference/must_visit/replace_day）及目标日仍待M4-B扩展，用途属于本轮，不是附件永久标签。跨表JSON引用由服务核对归属，不冒充外键约束。
...
当前聊天调用顺序：身份/会话与幂等/版本检查 → 读取统一历史与话题 → 携带附件时进入D识别并保存快照；无附件时继续本轮理解 → 地图、行程或资料回答；解析失败仍检索原话，内部输出格式无效转为资料建议 → 图外事务保存。整段会话删除是独立受认证HTTP接口，不开放给模型。D实际附件分支见12.5；M4-B再在同一行程图追加附件用途和识别结果，M4-C才增加独立审查、有限返工、人工暂停及持久恢复。详情见03第7节、09第13节。
