...
flowchart TD
    ...
    UI --> Status[workspace-status 仅会话所有者]
    Status --> Ledger
    UI --> Files[personal-files 仅本人]
...
**页面与调用职责：**`App.vue/chat.css`组织左侧历史、中间聊天和右侧状态/文件；`ThinkingProcess`展示公开过程，`WorkspaceStatus`读取实际统计，`PersonalFilesPanel`同时服务个人空间与当前会话侧栏。`DocumentPanel/DocumentSearch`组织管理员文件与检索测试。聊天侧栏默认360px，最小300px，上界为760px与当前工作区剩余空间共同限制；指针、方向键（20px，Shift为50px）、Home/End、双击复位共用边界，localStorage仅保存宽度偏好。工作区宽度小于900px使用带焦点限制、Escape关闭及焦点返回的抽屉。
...
**工具事件与历史JSON：**`events.py`在实际执行边界记录call_id，开始为running，结束为completed/failed/cancelled；可用正常响应表达的地图业务错误也标failed。没有调用的步骤不补造。`metrics.py`按call_id更新同一条记录，`RequirementHistoryService.append`把保存前取得的快照写入既有`requirement_turns.response_json.process`；`useRequirementConversation`恢复它，旧历史默认null。
...
| steps[].call_id | string/null，默认null；真实工具调用配对编号，普通阶段可空 |
