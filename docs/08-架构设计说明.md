...
> 最近源码核对：2026-09-15（Asia/Shanghai，M3口语偏好追问与补充字段容错）
> 范围：当前工作区 `backend/app/`、`frontend/src/` 与数据库迁移文件。  
> 验证口径：最新口语追问与字段容错见10.21，景点地址与定向补查契约见10.20，自动上传处理见10.19，元数据与后台基础见10.17，固定题库评测见10.18；本机已应用0006/0007。10.3至10.16保留历史小步验收，其中“尚未实现”描述仅对应当时。源码、自动检查和真实服务验收分别记录。
> 阅读配合：[07-各阶段开发说明](./07-各阶段开发说明.md) 讲开发顺序和逐文件写法；本文讲这些文件怎样组成系统。[02-技术设计文档](./02-技术设计文档.md) 包含目标设计，不能将其中尚未落地的模块视为当前能力。

> 2026-09-15目标设计核对：本次仅更新文档，新增第12节双角色/私人附件目标架构与数据字典，配套[09改造方案](./09-双角色与对话附件改造方案.md)。第1～11节继续记录现有源码和历史验收；当前没有账号授权、用户历史列表或多模态附件流程，本轮没有执行数据库迁移或重跑真实服务验收。
...
1. **M2 需求对话**：用户说话 → 读取已保存需求 → 模型提取本轮修改 → 程序校验与合并 → 强制检索知识库并生成带引用回答 → 保存完整一轮 → 页面显示；刷新页面时可以依据会话编号读取历史。
2. **M1 预算与草稿**：提交预算输入 → 程序计算预算 → 按需要保存需求快照和行程草稿 → 读取最新或指定版本。
3. **M3 资料与原文搜索**：选择文件 → 读取正文及位置 → 保存原文件与档案、预填城市/来源/待审核 → 自动提交后台任务 → 切片并建立索引 → 先按归属和标签限制资料，再做向量搜索 → 根据编号回查原文。后台索引由服务端连续分批执行，任务状态写入PostgreSQL，页面关闭后继续；服务中断后显式重试。文件读取和标签识别不调用模型；页面上传成功后自动索引调用Embedding，搜索也调用Embedding，主聊天使用证据生成回答。

**M1/M2共享旅行会话；M3资料属于独立本地资料库；主聊天把当轮引用原文快照关联到对话轮次。** 聊天完成后还不会自动计算预算或生成行程草稿。页面说“需求完整”，仅表示必要需求已齐全。
...
以下三张图采用浅色背景、白色卡片和竖向布局，按2026-09-15现有源码整理。总图覆盖M1预算与草稿、M2需求对话、M3知识库与RAG；它展示当前已实现能力，不表示M3所有计划功能均已完成。先看总图辨认分层，再看流程图了解一条消息或一份资料怎样处理。
...
新消息先检查历史和版本，再整理需求、检索原文、生成回答，最后保存整轮。已保存消息的重试直接返回原结果；网页补证与景点定位作为侧边分支。知识库和网页均无证据时跳过知识生成并说明无法确认，检索或模型故障则报错、不保存半轮。
...
| `frontend/src/components/DocumentPanel.vue` | 组合上传组件、按文件名筛选列表、原文/片段切换 | 资料柜的操作面板 |
...
| `frontend/src/components/DocumentSearch.vue` | 分批索引、进度、暂停续建和全库搜索 | 为资料建立目录，再按含义查原文；App传递页签活动状态以停止隐藏页的后续批次 |
...
6. 进入强制RAG链：根据本轮消息和近期话题检索资料，回查原文，再让DeepSeek根据证据生成并校验引用。
7. 抽取和RAG完成后才进入追加事务，锁住会话行，检查版本并写入回答与引用整轮快照。
8. 提交成功后返回；知识问答只显示自然回答，只有有效需求状态才更新右侧面板。
...
`requirement_turns.response_json` 保存整个聊天响应，原话在 `result.original_message` 中；一行是一整轮，而不是用户消息和助手消息分别各存一行。第五小步新增可选knowledge快照，包含回答状态、要点、来源原文、网页证据、景点卡片与地图结果；旧行读取时默认null，无需更新旧行或迁移表。
...
源码包含`0001_business_tables`至`0007_document_jobs`。2026-09-15本机PostgreSQL已升级到0007，alembic check无差异；0006增加documents的四个标签，0007增加document_jobs，合计七张业务表。迁移往返、待清理状态阻止回滚及ORM一致性在临时schema验证。真实业务库只升级，回滚只在随机临时schema中测试。

当前就绪检查代码检查七张业务表及字段，不核对CHECK约束或迁移版本，因此仍须以alembic current/check确认，不能只看ready。数据库断开时应用保留存活检查，就绪接口返回503；首次恢复连接后的任务请求补做启动时未完成的中断识别。

### 7.5 当前表的详细数据字典

以下按2026-09-15的ORM和`0001`至`0007`迁移文件核对，本步已连接数据库执行迁移。
...
| `response_json` | JSONB | 整轮响应及knowledge原文/网页/景点/地图快照，非空、无默认值；CHECK要求JSON对象。旧快照可无knowledge字段 |
...
| `owner_id` | VARCHAR(100) | 否 | 无 | 服务端当前固定`local-demo`；预留归属，不是已实现账号认证 |
...
| `source` | VARCHAR(255) | 是 | 无 | 人工来源名称或链接，不代表来源权重 |
| `review_status` | VARCHAR(20) | 是 | 无 | pending/approved/rejected，NULL为未标注；新上传由业务层填写pending |
| `poi_id` | VARCHAR(100) | 是 | 无 | 资料级人工外部地点编号，不是地图已核验结果；多景点资料可留空 |
...
主键：`id`。唯一约束：`storage_name`，以及`uq_documents_owner_hash(owner_id, content_hash)`。普通复合索引：`ix_documents_owner_created(owner_id, created_at)`，服务按创建时间和ID倒序分页。无外键、无会话关联、无账号表关联。
...
**元数据约束：**`ck_documents_review_status`允许NULL或pending/approved/rejected。四个标签没有外键、唯一约束或新增索引，沿用`ix_documents_owner_created`；文本长度由字段及输入格式限制，空白规范化为NULL。迁移不回填旧资料；新上传按规则预填，旧资料由用户点击补全空值。数据库默认值仍为空，与业务层填写pending区分。
...
| `owner_id` | VarChar(100) | 否／无 | 资料归属，当前后端固定local-demo；检索强制过滤，不允许前端传值 |
...
| `PATCH /documents/{id}/metadata` | 部分更新四个标签；未传保持、null清空 | 资料页标签编辑 |
...
| `GET /documents?limit=50&offset=0&q=关键词` | 先按归属和文件名筛选再分页；q可省略、最多255字，limit为1～100，offset≥0 | 资料页使用 |
...
主聊天仍使用`POST /sessions/{session_id}/requirement-messages`，不新增独立问答路由。请求原话1～6000字符、message_id和expected_revision合同不变。响应新增`knowledge`（旧历史为null）及`status=knowledge`；needs_clarification/complete表示有效需求，knowledge轮次不覆盖需求卡片。
...
引用或JSON不合法返回502/HTTP_502；DeepSeek故障503/MODEL_UNAVAILABLE；检索故障503/VECTOR_UNAVAILABLE或数据库统一错误。均不保存半轮，不自动改为自由回答，网络故障不自动重试；格式或证据校验失败可让模型修正一次，再失败仍返回502；地图结果后的第二次生成属于正常补查流程，整轮知识生成最多3次。对用户使用自然表达，不出现“资料中”“根据知识库”等来源套话；资料不足的reply为“这部分信息目前还无法确认，暂时不能给出可靠结论。”。模型生成的事实限于sources、web_search与实际返回的地图结果；地图人均消费不能改写为门票。本地与网页均没有证据时不调用回答模型；登记用户条件仍返回程序补问，同时直接说明知识部分无法确认，以免混合问题被静默遗漏。已有消息编号的重试与GET历史只读取旧快照。
...
2. `api/document/routes.py`接收文件，注入数据库、上传目录及固定`local-demo`归属。同步接口在线程池中运行读取任务。
...
源码调用链：`DocumentSearch.vue → api/document/search.py → DocumentSearchService → EmbeddingClient + MilvusStore → PostgreSQL原文回查`。当前分支`codex/m3`，未提交或推送。没有增加Python依赖，复用httpx调用Milvus REST v2。前端可建立索引、暂停、刷新续建、跨资料搜索并打开命中文件。
...
**当前调用链：**`App/useRequirementConversation → api/requirement/history.send_saved_message → chat/service.process_saved_message → build_requirement_response需求提取 → chat/rag.ground_chat_response → DocumentSearchService → document/answer.answer_from_sources → DeepSeekClient → RequirementHistoryService.append`。主聊天每轮必须检索，无开关、无额外问答按钮。依据用户最终要求，聊天不显示引用编号/原文展开区；knowledge仅用于后台校验和证据快照。前端恢复旧历史时，只隐藏已知证据编号，不改写旧数据库记录。纯需求补问是程序规则，不从资料猜用户条件；知识性回答只能经过证据生成与引用校验。旧的无状态requirements接口仍只提取需求，不提供知识回答。
...
**源码调用链（按10.11更新）：**主聊天每轮检索知识库，景点相关问题扩展介绍与门票检索词，最多保留5段原文；需要知识回答时补查允许域名内的网页。第一遍模型生成景点草稿，maps.py为每张卡片核对地点，即使已有历史门票也定位。城市与景点名必须同时出现在指定证据中，最多3处、同城同名去重；查询后生成最终回答；整轮可额外修正一次格式，不能再申请地图或增加未定位的卡片。本地和网页均无证据时不生成；地图失败允许保留有依据的介绍。
...
**当前调用链：**需求提取 → 主聊天检索 → 明确模糊距离先追问；否则生成景点/自然追问 → 程序核对主体并整理可选详情 → 地图/网页补查 → 再次核对 → 保存回答和clarification → 页面卡片与追问同时显示。检索和模型服务本身故障仍按原接口报错，不混写成用户条件不明确。
...
设计核对日期：2026-09-15。本节是本次需求对应的拟议结构，不是当前数据字典；第7.5节继续说明已实现字段。当前源码迁移链至0007，本节所有新增/变更迁移均未创建、未执行，实际编号在实施时核对head后分配。产品和交互详情见[09方案](./09-双角色与对话附件改造方案.md)。

### 12.1 职责与调用链

| 边界 | 目标调用顺序 | 复用与新增 |
|---|---|---|
| 登录与身份 | 登录接口 → 密码校验 → 服务端登录态 → 后续请求解析当前用户 | 新增users/auth_sessions及身份依赖 |
| 私人会话 | 当前用户 → sessions.user_id归属检查 → 历史/需求/草稿服务 | 复用sessions、requirement_turns、travel_requests、itineraries |
| 管理员知识库 | 管理员校验 → 上传/两标签 → 解析/切片/索引 → 共享知识库 | 复用services/document；不按上传管理员划分可见文件 |
...
目标架构图在02第3节同步维护；本文件第2节三张现有SVG/PNG继续表示当前源码，本次没有改写为已具备双角色。实施对应小步后再同步替换当前图和当前职责描述。

### 12.2 users（拟新增）

| 字段 | PostgreSQL类型 | 可空 | 默认值 | 含义 |
|---|---|---|---|---|
| id | UUID | 否 | 应用uuid4 | 主键 |
| username | VARCHAR(50) | 否 | 无 | 登录名，保存前去首尾空白并统一大小写 |
| password_hash | VARCHAR(255) | 否 | 无 | 密码哈希，不保存明文 |
| role | VARCHAR(20) | 否 | 数据库'user' | user普通用户、admin管理员 |
| is_active | BOOLEAN | 否 | 数据库true | 是否允许登录和继续使用已有登录态 |
| created_at | TIMESTAMPTZ | 否 | 数据库now() | 创建时刻 |
| updated_at | TIMESTAMPTZ | 否 | 数据库now()，更新由服务写入 | 账号修改时刻 |

主键id；username唯一，额外CHECK保证非空且等于lower(trim(username))；role CHECK仅允许user/admin。唯一约束已提供用户名索引，不另建重复索引。role由部署管理流程设置，登录请求不能自选角色；本轮无公开注册或改角色接口。默认方案为部署命令开通账号，不提前增加email、头像或长期偏好。

### 12.3 auth_sessions（拟新增）

| 字段 | PostgreSQL类型 | 可空 | 默认值 | 含义 |
|---|---|---|---|---|
| id | UUID | 否 | 应用uuid4 | 登录态主键，与旅行session_id不同 |
| user_id | UUID | 否 | 无 | 外键users.id |
| token_hash | VARCHAR(64) | 否 | 无 | 随机登录令牌的SHA-256摘要，原令牌仅交给Cookie |
| expires_at | TIMESTAMPTZ | 否 | 无，由服务设置 | 登录有效期截止时刻 |
| created_at | TIMESTAMPTZ | 否 | 数据库now() | 创建时刻 |

主键id；token_hash唯一及长度CHECK；expires_at > created_at。外键默认限制删除用户，账号停用不直接删除业务数据。user_id索引用于撤销用户登录态，expires_at索引用于过期清理；退出时删除对应登录态即可，无需另存revoked字段。每次请求检查账号is_active，不能只在登录时检查。

### 12.4 sessions（拟扩展）

| 字段 | PostgreSQL类型 | 可空 | 默认值 | 含义 |
|---|---|---|---|---|
| user_id | UUID | 迁移初期是，目标否 | 无，服务端按登录身份写入 | 外键users.id，旅行会话所有者 |

其余id/thread_id/title/status/created_at/updated_at字段与现有约束沿用7.5字典；不新增第二套会话表。增加(user_id, updated_at DESC, id DESC)历史分页索引；按updated_at/id游标翻页，返回title/status/updated_at等摘要，不返回别人的会话。首次成功轮次更新标题和updated_at，后续成功轮次/草稿保存继续更新。

需求、轮次、草稿的现有session_id外键和唯一约束保留，不在每张子表重复存user_id；服务查询通过会话强制归属。users→sessions外键不级联删除；旧会话显式认领后才设user_id非空，具体迁移见09第9节。
...
| 字段 | PostgreSQL类型 | 可空 | 默认值 | 目标含义 |
|---|---|---|---|---|
| owner_id | VARCHAR(100)，沿用 | 否 | 无，服务固定注入knowledge-base | 共享知识库范围，不是登录账号ID |
| city | VARCHAR(100)，沿用 | 是 | NULL | 唯一城市标签，无法确定则未标注 |
| category | VARCHAR(20)，新增 | 是 | NULL | accommodation/attraction/restaurant，对应住宿/景点/餐馆 |
| uploaded_by | UUID，新增 | 是 | NULL | 外键users.id，记录上传管理员；历史上传者不明可空 |

category CHECK仅允许上述三值或NULL；新增(city, category, created_at, id)索引支持城市/类别文件分页。保留(owner_id, content_hash)去重唯一约束与原片段外键；uploaded_by引用用户时ON DELETE SET NULL，不让账号变化删除共享知识库。新上传由服务强制写当前管理员id；数据库外键仅保证用户存在，管理员身份由服务校验。

source/review_status/poi_id退出业务合同与所有新筛选；清理迁移才删除旧列和ck_documents_review_status。原文位置、文件信息、历史引用以及地图返回POI编号继续保留。category只是PostgreSQL过滤条件，修改标签不改变片段文本或向量，无需重新Embedding。

owner_id范围迁移需同步Milvus：暂停管理写入，改写既有向量的范围并确认读回，再切换关系库与检索服务；出现失败保持维护状态。不能只改PostgreSQL导致旧向量搜索不到，也不能把客户端user_id作为知识库范围。详见09第9节。
...
| 阶段 | 目标结构变化 | 本轮状态 |
|---|---|---|
| A | 新建users/auth_sessions、sessions.user_id回填与非空约束、历史索引 | 未创建迁移，未执行 |
| C | documents.category/uploaded_by、两标签索引、共享范围数据迁移；旧标签列延后清理 | 未创建迁移，未执行 |
| D | conversation_attachments表与约束；轮次JSON兼容扩展 | 未创建迁移，未执行 |
| E | 复用需求/草稿表，扩展逐日JSON合同及事务保存 | 未实现；合同扩展不必新增表 |

上述E的逐日合同和事务保存提前在M4-A建立，E/M4-B复用并增加附件输入。M4-A在A～C基础完成后启动；M4-C才接持久检查点和人工暂停恢复。下节为本次追加方案，全部未实施。

验收需覆盖两个用户各多个会话、管理员管理不同上传人的知识库、全部旧管理URL无权限绕过、历史认领、城市分组/分页、私有附件读取及不入向量库、路线模糊追问、草稿局部修改、重试幂等和并发冲突。本次只有文档一致性/链接/格式检查，不将第10节历史测试数字复用为新设计验证。
...
现有(owner_id, content_hash)去重继续有效，重复内容返回提示，不为替换复制记录或静默切换其他资料组。此组字段、索引及切换数据迁移只在F单独创建；本轮没有新增Alembic文件，亦无运行验收。

### 12.10 智能体当前状态与目标接入

2026-09-15重新核对：services/chat/service.py仍以普通函数固定串联抽取、ground_chat_response和保存；正式app未发现LangGraph StateGraph/checkpointer调用，pyproject.toml未直接声明LangGraph依赖。已有检索、网页和地图能力可复用，不等于已完成M4工具选择、规划或恢复验收。
