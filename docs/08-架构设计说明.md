...
`TravelGraphState` 保存 payload、candidate（回复、候选、旧草稿和版本）、attempts、review、feedback、route、result、cancelled；不序列化模型、连接或工具闭包。候选仅经规则和独立审查后进入原保存事务。等待消息另存为历史；恢复携带执行号及 continue/accept/cancel，旧轮次不可继续，失败重试必须复用原请求。accept 仅允许程序校验通过且审查结论为 choice 的可见候选。
