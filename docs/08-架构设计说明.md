...
阶段时间仍只属于运行事件和评测报告；逐次调用现另存0015账本，见12.23。前端过程时间仅保留当次运行，刷新恢复对话正文而不恢复metrics；管理员费用页读取独立账本，不能替代供应商实际账单。
...
**新增SQL表：**迁移0015_usage_events，前置0014_chat_workflows；正式数据库已执行0015。字段均无server default，不含正文、密钥或用户/会话外键；删除聊天不删除费用。没有额外唯一/CHECK约束，JSON结构由写入服务维护。
...
metrics新增known_costs、unknown_cost_calls、unpersisted_calls、usage_scope=external_api_calls；model_calls为兼容保留名称，现也含非模型调用。阶段耗时仍不写消息历史；费用独立持久化。API计量不同，网页/地图按次数、OCR按页数，不能将缺token当零费用。
...
调用失败→usage_call保存异常类名/HTTP状态→提供方适配器归类→网关按原策略发出fallback→前端过程区显示具体原因。HTTP 429表示请求频率或token吞吐受限，不能写成余额不足；未知类别仍用通用提示。只显示程序定义的错误类别，不透出供应商异常原文。过程步骤仍仅在本次页面保留，刷新后的历史不补造失败原因；后台账本独立持久保存新增安全诊断字段。JSON增加可选字段，无SQL表结构、索引或迁移变化。
