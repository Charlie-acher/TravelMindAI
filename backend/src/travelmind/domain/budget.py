"""
确定性预算计算：输入条件，返回能逐项复算的人民币预算。

所有单价均为 demo-cny-v1 演示假设，不是实时酒店、门票或交通报价。
此模块只依赖 Python 标准库，调用它不需要网络或 API Key。
"""

from dataclasses import dataclass
from decimal import ROUND_HALF_UP, Decimal

# Decimal 对应 Java BigDecimal。用字符串创建，避免把 float 的误差带进来。
CENT = Decimal("0.01")
...
class BudgetValidationError(ValueError):
    """表示输入不符合预算规则；API 层稍后把它转换为可读的 HTTP 错误。"""


@dataclass(frozen=True)
class BudgetEstimate:
    """计算结果，类似 Java DTO；dataclass 自动生成构造函数和相等比较。

    frozen=True 防止重新赋值字段，但不是深度冻结：调用方仍不应修改字典内容。
    该对象不包含 HTTP request_id，领域计算不关心请求来自哪里。
    """
    ...
    price_version: str
    # 各项目的演示单价，键是项目及计费单位，值是人民币金额。 （多项名称➡金额）
...
"""
按天数、人数、总预算和住宿档位估算全团费用。

* 表示后面的参数必须写名字，例如 days=3，避免人数和天数位置传反。
金额参数要求 Decimal；HTTP JSON 的转换由后续 Pydantic DTO 负责。
这里再次校验，是为了以后 Agent 直接调用函数时也受相同规则约束。
"""
def calculate_budget(
    *, days: int, travelers: int, total_budget: Decimal, lodging: str = "economy"
) -> BudgetEstimate:
    """计算总预算，返回 BudgetEstimate 对象。"""
    ...
    costs = {
        ...
        "intercity_transport": unit_prices["intercity_per_person_trip"] * travelers, # 总城际交通总费用 = 每人的全程城际交通费 × 人数。
